...
public sealed class WebSocketClientConnection : IClientConnectionSink {
    ...
    public ValueTask SendBinaryAsync(ReadOnlyMemory<byte> message, CancellationToken ct = default) {
        return SendCoreAsync(message, WebSocketMessageType.Binary, ct);
    }
    ...
    private async ValueTask SendCoreAsync(ReadOnlyMemory<byte> payload, WebSocketMessageType type,
        CancellationToken ct) {
        var connection = Connection;
        if (_socket.State != WebSocketState.Open) throw new ClientConnectionClosedException(connection.ConnectionId);

        await _sendLock.WaitAsync(ct);
        try {
            try {
                await _socket.SendAsync(payload, type, true, ct);
            }
