...
namespace Elarion.Blobs.Tus.PostgreSql.Generators;

/// <summary>
/// Fills every partial <c>DbContext</c> annotated with <c>[GenerateElarionTusStorage]</c> with the
/// <c>DbSet&lt;TusUploadRow&gt;</c> property and an implementation of the EF generator's per-feature
/// model-configuration seam that calls <c>UseElarionTusStorage</c>. So the host writes neither the DbSet
/// nor the table mapping — mirroring <c>[GenerateElarionBlobStorage]</c>.
/// </summary>
[Generator(LanguageNames.CSharp)]
public sealed class ElarionTusStorageGenerator : IIncrementalGenerator {
    private const string AttributeMetadataName = "Elarion.Blobs.Tus.PostgreSql.GenerateElarionTusStorageAttribute";
    private const string GenerateDbSetsAttributeName = ElarionGeneratorConventions.GenerateDbSetsAttribute;
    private const string TusNamespace = "global::Elarion.Blobs.Tus.PostgreSql";
    ...
    private static readonly string SeamMethodName =
        ElarionGeneratorConventions.ModelConfigurationSeamName("GenerateElarionTusStorageAttribute");

    private static readonly DiagnosticDescriptor MissingGenerateDbSets = new(
        "ELTUS001",
        "[GenerateElarionTusStorage] requires [GenerateDbSets]",
        "Context '{0}' is annotated with [GenerateElarionTusStorage] but not [GenerateDbSets]; add [GenerateDbSets] "
        + "so the tus staging DbSet and model-configuration seam are generated",
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context) {
        var targets = context.SyntaxProvider
            ...
            .WithTrackingName("TusStorageTargets");
    ...
    private sealed record TusStorageTarget(
        string Namespace,
        string ContextName,
        string HintName,
        bool SnakeCase,
    ...
    private static TusStorageTarget? GetTarget(GeneratorAttributeSyntaxContext ctx) {
    ...
    private static void Emit(SourceProductionContext spc, TusStorageTarget target) {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine("// Source: Elarion.Blobs.Tus.PostgreSql.Generators.ElarionTusStorageGenerator");
        ...
        sb.AppendLine("{");
        sb.AppendLine($"    public DbSet<{TusNamespace}.TusUploadRow> TusUploads => Set<{TusNamespace}.TusUploadRow>();");
        sb.AppendLine();
        ...
        sb.AppendLine($"        {TusNamespace}.PostgreSqlTusStorageModelBuilderExtensions.UseElarionTusStorage(");
        sb.AppendLine(
        ...
        spc.AddSource($"{target.HintName}.ElarionTusStorage.g.cs", SourceText.From(sb.ToString(), Encoding.UTF8));
    }
}
