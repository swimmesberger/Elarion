...
/// operations flagged <see cref="Elarion.Abstractions.HandlerTransports.Mcp"/>, so an MCP-only handler is
/// dispatchable here yet absent from <c>/rpc</c> and the JSON-RPC schema, a JSON-RPC-only handler is absent
/// here, and a "both" handler is
...
/// <see cref="RpcToolInvoker"/> filtered by <see cref="Elarion.Abstractions.HandlerTransports.Mcp"/>. It carries the
...
public sealed class McpDispatcher {
    ...
    /// <summary>The shared registry; MCP serves the subset flagged <see cref="Elarion.Abstractions.HandlerTransports.Mcp"/>.</summary>
