...
public sealed class KeysetGenerator : IIncrementalGenerator {
    ...
    /// <summary>
    /// Finds the entity's primary-key property for the deterministic-keyset check (ELKEY004): a property
    /// carrying <c>[Key]</c>, else the EF conventional key named <c>Id</c> or <c>{EntityTypeName}Id</c>.
    ...
    private static void Emit(SourceProductionContext context, KeysetTarget target, ProviderKind provider) {
        ...
        var identityTag = ComputeIdentityTag(target);
        ...
        sb.AppendLine();
        // Stable keyset-identity tag (FNV-1a of the definition FQN and ordered column types). Embedded in
        // every cursor this keyset mints and verified on decode so a cursor from another endpoint is
        // rejected instead of silently reinterpreted as a garbage seek position.
        sb.AppendLine($"    private const uint __CursorTag = {identityTag}u;");
        sb.AppendLine();
    ...
    /// <summary>
    /// Computes a build-stable identity tag for the keyset from its definition FQN and its ordered
    /// column names, directions, and CLR types. FNV-1a over UTF-16 code units — deterministic across
    ...
    private static uint ComputeIdentityTag(KeysetTarget target) {
        ...
        static void Mix(ref uint hash, string value) {
            ...
            // A field separator so ("ab","c") and ("a","bc") tag differently.
    ...
    private static void EmitBuildSeek(StringBuilder sb, KeysetTarget target, string entity, bool useRowValues) {
        ...
        sb.AppendLine("        {");
        sb.AppendLine($"            throw new {KeysetNamespace}.MalformedCursorException();");
        sb.AppendLine("        }");
        sb.AppendLine();
