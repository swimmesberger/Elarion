...
public sealed class DbContextGenerator : IIncrementalGenerator {
    ...
    private static readonly DiagnosticDescriptor DuplicateDbSetName = new(
        "ELEFC002",
        "Duplicate DbSet property name",
        "Entities '{0}' and '{1}' both map to DbSet property '{2}' on context '{3}'; rename one entity "
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context) {
        ...
        context.RegisterSourceOutput(configResults, static (spc, result) => {
            if (result.Diagnostic is { } diagnostic) spc.ReportDiagnostic(diagnostic.ToDiagnostic());
        });
    ...
    private static TargetInfo? GetClassTarget(GeneratorAttributeSyntaxContext ctx) {
        if (ctx.TargetSymbol is not INamedTypeSymbol classSymbol || classSymbol.IsAbstract) return null;
        ...
        if (dbContext is null || !InheritsFrom(classSymbol, dbContext)) return null;
    ...
    private static ReferencedData ReadManifest(MetadataReference reference, CancellationToken ct) {
        ...
        foreach (var (key, value) in EntityConfigurationManifest.ReadEntries(reference, ct)) {
            if (key != EntityConfigurationManifest.ConfigKey) continue;

            if (EntityConfigurationManifest.TryDecodeConfig(value, out var fullName, out var ns, out var scopes,
                    out var entities)) {
    ...
    private static EquatableArray<ConfigInfo> Merge(
        ...
        return configs.ToImmutable()
            .Sort(static (a, b) => string.Compare(a.FullName, b.FullName, StringComparison.Ordinal));
    }

    private static ImmutableArray<ConfigInfo>
        SelectActiveConfigs(EquatableArray<ConfigInfo> configs, TargetInfo target) {
        ...
        if (target.Scopes.IsEmpty) return configs.AsImmutableArray;
    ...
    private static void GenerateDbContextClass(
        ...
        foreach (var entity in entities) {
            ...
            sb.AppendLine(string.Format("    public DbSet<global::{0}> {1} => Set<global::{0}>();", entity.FullName,
                pluralName));
        }
        ...
        sb.AppendLine();
        sb.AppendLine(
            "        // Extensibility seam: hand-written model configuration can implement this partial method.");
        ...
        if (!entities.IsEmpty) {
            sb.AppendLine();
            sb.AppendLine(
                "        // Last, so it also covers entities the seams above added (and their navigation-discovered children).");
        ...
        sb.AppendLine("    /// <summary>");
        sb.AppendLine(
            "    /// Extensibility seam invoked at the end of <see cref=\"ConfigureEntities\"/> so other source");
        ...
        if (!entities.IsEmpty) AppendClientAssignedGuidKeys(sb, entities);
    ...
    private static void AppendClientAssignedGuidKeys(StringBuilder sb, ImmutableArray<EntityInfo> entities) {
        ...
        sb.AppendLine("    /// <summary>");
        sb.AppendLine(
            "    /// Declares each domain entity's single-property <c>Guid</c> primary key as client-assigned");
        sb.AppendLine(
            "    /// (<c>ValueGenerated.Never</c>): Elarion entities own their ids and mint them in code (for example");
        sb.AppendLine(
            "    /// <c>Guid.CreateVersion7()</c>), so the model must not claim generation — a set value on a \"generated\"");
        sb.AppendLine(
            "    /// key makes EF Core's insert-vs-update heuristic treat a new child added to a tracked parent as an");
        sb.AppendLine(
            "    /// update (UPDATE … 0 rows → DbUpdateConcurrencyException). Scoped to the assemblies of the discovered");
        sb.AppendLine(
            "    /// [EntityConfiguration] entities, so navigation-discovered children are covered while Identity and");
        sb.AppendLine(
            "    /// framework entities keep their packaged generation. Explicit or data-annotation configuration, a");
        ...
        sb.AppendLine(
            "        var domainAssemblies = new global::System.Collections.Generic.HashSet<global::System.Reflection.Assembly> {");
        foreach (var entity in entities)
        ...
        sb.AppendLine();
        sb.AppendLine(
            "        foreach (var entityType in global::System.Linq.Enumerable.ToList(modelBuilder.Model.GetEntityTypes())) {");
        sb.AppendLine(
            "            // Derived types share the root's key; owned/keyless and composite keys are out of scope.");
        sb.AppendLine(
            "            if (entityType.BaseType is not null || !domainAssemblies.Contains(entityType.ClrType.Assembly)) {");
        ...
        sb.AppendLine();
        sb.AppendLine(
            "            // Only override EF's convention claim: explicit or data-annotation configuration wins.");
        sb.AppendLine("            var valueGeneratedSource =");
        sb.AppendLine(
            "                ((global::Microsoft.EntityFrameworkCore.Metadata.IConventionProperty)property).GetValueGeneratedConfigurationSource();");
        sb.AppendLine("            if (valueGeneratedSource is not null &&");
        sb.AppendLine(
            "                valueGeneratedSource != global::Microsoft.EntityFrameworkCore.Metadata.ConfigurationSource.Convention) {");
        sb.AppendLine("                continue;");
        sb.AppendLine("            }");
        sb.AppendLine();
        sb.AppendLine(
            "            // A configured value generator or store default is a deliberate generation choice — leave it alone.");
        ...
        sb.AppendLine();
        sb.AppendLine(
            "            property.ValueGenerated = global::Microsoft.EntityFrameworkCore.Metadata.ValueGenerated.Never;");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
    }
