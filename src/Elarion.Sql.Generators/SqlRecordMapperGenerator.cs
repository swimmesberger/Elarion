...
public sealed partial class SqlRecordMapperGenerator : IIncrementalGenerator {
    ...
    private static SqlRecordModel? GetModel(GeneratorAttributeSyntaxContext ctx) {
        ...
        if (diagnostics.Count > 0) return Invalid();

        var bound = BindConstructor(type, columns, diagnostics);
        if (bound is null) return Invalid();

        var resolvedTable = tableName ?? ToSnakeCase(typeName);
        var selectSql = "SELECT " + string.Join(", ", bound.Select(static c => c.ColumnName)) + " FROM " +
                        resolvedTable;
        ...
        if (!isPartial) diagnostics.Add(DiagnosticInfo.Create(NotPartial, LocationInfo.From(type), typeFqn));
    ...
    private static ColumnModel? MapColumn(
        ...
        if (isJson) {
            ...
            return new ColumnModel(
                ...
                "string", null, jsonType,
                isNullable, isValueType, -1);
        }

        if (effective.TypeKind == TypeKind.Enum &&
            effective is INamedTypeSymbol { EnumUnderlyingType: { } underlying }) {
            var underlyingRead = TryMapScalar(underlying);
            if (underlyingRead is not null)
                return new ColumnModel(
                    property.Name, columnName, ColumnKind.Enum,
                    underlyingRead, effective.ToDisplayString(fmt), null,
                    isNullable, isValueType, -1);
        }
    ...
    private static string? TryMapScalar(ITypeSymbol type) {
        ...
        if (type is IArrayTypeSymbol { Rank: 1, ElementType.SpecialType: SpecialType.System_Byte }) return "byte[]";
    ...
    private static List<ColumnModel>? BindConstructor(
        ...
        foreach (var column in columns) writable[column.PropertyName] = IsWritable(type, column.PropertyName);
        ...
        if (hasParameterless && columns.All(column => writable[column.PropertyName])) return columns;
        ...
        foreach (var ctor in accessibleCtors.OrderByDescending(static ctor => ctor.Parameters.Length)) {
            ...
            if (columns.Any(column =>
                    !positions.ContainsKey(column.PropertyName) && !writable[column.PropertyName])) continue;
        ...
        diagnostics.Add(DiagnosticInfo.Create(
            NoUsableConstructor, LocationInfo.From(type),
            type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)));
        return null;

        static bool IsWritable(INamedTypeSymbol type, string propertyName) {
            return FindProperty(type, propertyName)?.SetMethod is
                { DeclaredAccessibility: Accessibility.Public or Accessibility.Internal };
        }
