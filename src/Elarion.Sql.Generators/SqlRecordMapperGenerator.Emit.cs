...
public sealed partial class SqlRecordMapperGenerator {
    private static void EmitMapper(SourceProductionContext context, SqlRecordModel model, ProviderModel provider) {
        ...
        builder.AppendLine($$"""
                             ...
                             """);
        EmitBindBody(builder, model, provider.Kind);
        builder.AppendLine("    }");

        // Binary-COPY members (ADR-0068): only under the Npgsql trigger with Elarion.Sql.PostgreSql
        // referenced. Column writes use the same CLR-type inference as BindParameters (explicit
        // NpgsqlDbType only for jsonb), so COPY and INSERT have identical value semantics.
        if (provider.NpgsqlCopy) {
            var copy =
                $"COPY {model.TableName} ({string.Join(", ", columns.Select(static c => c.ColumnName))}) "
                + "FROM STDIN (FORMAT BINARY)";
            builder.AppendLine($$"""

                                     /// <summary>The binary-COPY command over all mapped columns (ADR-0068).</summary>
                                     public const string Copy = {{SourceLiterals.String(copy)}};

                                     /// <summary>
                                     /// Starts a row on the importer and writes every mapped column in declaration order,
                                     /// matching <c>BindParameters</c> value semantics.
                                     /// </summary>
                                     public async global::System.Threading.Tasks.Task WriteRowAsync(
                                         global::Npgsql.NpgsqlBinaryImporter importer, {{type}} row,
                                         global::System.Threading.CancellationToken cancellationToken = default) {
                                         await importer.StartRowAsync(cancellationToken).ConfigureAwait(false);
                                 """);
            foreach (var column in columns) EmitCopyColumnWrite(builder, column);
    ...
    private static void EmitCopyColumnWrite(StringBuilder builder, ColumnModel column) {
        var value = "row." + column.PropertyName;
        var access = column.IsNullable && column.IsValueType ? value + ".Value" : value;
        var write = column.Kind switch {
            ColumnKind.Json =>
                $"await importer.WriteAsync(global::System.Text.Json.JsonSerializer.Serialize({access}, JsonTypeInfo{column.PropertyName}), global::NpgsqlTypes.NpgsqlDbType.Jsonb, cancellationToken).ConfigureAwait(false);",
            ColumnKind.Enum =>
                $"await importer.WriteAsync(({column.ReadTypeFqn}){access}, cancellationToken).ConfigureAwait(false);",
            _ => $"await importer.WriteAsync({access}, cancellationToken).ConfigureAwait(false);"
        };
        ...
        builder.AppendLine($"        if ({value} is null) await importer.WriteNullAsync(cancellationToken).ConfigureAwait(false);");
        builder.AppendLine($"        else {write}");
    }
    ...
    // hand-written SQL composes without :raw. The row type must be partial (ELSQL010). Under the
    // Npgsql COPY trigger (ADR-0068) the partial additionally implements INpgsqlCopyRecord<T>.
    private static void EmitSelfMapping(SourceProductionContext context, SqlRecordModel model, ProviderModel provider) {
        var builder = new StringBuilder();
        builder.AppendLine("// <auto-generated/>");
        builder.AppendLine("#nullable enable");
        builder.AppendLine();
        ...
        var emitCopy = provider.NpgsqlCopy && model.EmitCopyMapping;
        var interfaces = emitCopy
            ? $"global::Elarion.Sql.ISqlRecord<{model.TypeName}>, global::Elarion.Sql.INpgsqlCopyRecord<{model.TypeName}>"
            : $"global::Elarion.Sql.ISqlRecord<{model.TypeName}>";

        builder.AppendLine($$"""
                             partial {{model.TypeKeyword}} {{model.TypeName}} : {{interfaces}} {
        ...
        if (emitCopy)
            builder.AppendLine($$"""
                                     ...
                                     /// <summary>The binary-COPY command over all mapped columns (backs the bulk insert helpers).</summary>
                                     public static string CopyCommandText => {{mapperRef}}.Copy;

                                     /// <summary>The mapped table name, for composing the staged-upsert statements.</summary>
                                     public static string CopyTableName => {{mapperRef}}.TableName;

                                     /// <summary>All mapped column names, comma-separated, in COPY write order.</summary>
                                     public static string CopyColumnList => {{mapperRef}}.Columns.All;

                                     /// <summary>Writes one row to a binary importer via the generated mapper.</summary>
                                     public static global::System.Threading.Tasks.Task WriteRowAsync(
                                         global::Npgsql.NpgsqlBinaryImporter importer, {{model.TypeName}} row,
                                         global::System.Threading.CancellationToken cancellationToken) =>
                                         {{mapperRef}}.Instance.WriteRowAsync(importer, row, cancellationToken);
                                 """);
