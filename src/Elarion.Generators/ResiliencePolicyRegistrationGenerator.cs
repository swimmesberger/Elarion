...
public sealed class ResiliencePolicyRegistrationGenerator : IIncrementalGenerator {
    ...
    private static readonly DiagnosticDescriptor InvalidPolicyDescriptor = new(
        ...
        "Resilience policy '{0}' is invalid: {1}",
    ...
    private static readonly DiagnosticDescriptor DuplicatePolicyNameDescriptor = new(
        "ELRES002",
        "Duplicate resilience policy name",
        "Resilience policy name '{0}' is used more than once",
        "Elarion.Abstractions.Resilience",
        DiagnosticSeverity.Error,
        true);
    ...
    private static TimeoutInfo? ParseTimeout(
        ...
        if (timeoutText is null) return null;
    ...
    private static string GenerateSource(string assemblyName, IReadOnlyList<PolicyInfo> policies) {
        ...
        sb.AppendLine("{");
        sb.AppendLine(
            $"    public static global::Microsoft.Extensions.DependencyInjection.IServiceCollection {registrationMethod}(this global::Microsoft.Extensions.DependencyInjection.IServiceCollection services)");
        sb.AppendLine("    {");
        foreach (var policy in policies.OrderBy(static policy => policy.Name, StringComparer.Ordinal)) {
    ...
    private static void AppendPolicyType(StringBuilder sb, PolicyInfo policy) {
        ...
        sb.AppendLine();
        sb.AppendLine(
            "    public static global::Elarion.Abstractions.Resilience.ResiliencePolicyReference Reference { get; } = new() { Name = Name };");
        sb.AppendLine();
        sb.AppendLine(
            $"    public static global::Microsoft.Extensions.DependencyInjection.IServiceCollection Add{policy.TypeName}(this global::Microsoft.Extensions.DependencyInjection.IServiceCollection services)");
        sb.AppendLine("    {");
    ...
    private static void AppendMetadataRegistration(StringBuilder sb, PolicyInfo policy) {
        sb.AppendLine(
            "        services.AddSingleton(new global::Elarion.Abstractions.Resilience.ResiliencePolicyMetadataRegistration");
        sb.AppendLine("        {");
        ...
        if (policy.Retry is { } retry) {
            ...
            sb.AppendLine(
                $"                    Delay = global::System.TimeSpan.FromTicks({retry.DelayTicks.ToString(CultureInfo.InvariantCulture)}L),");
            sb.AppendLine(
                $"                    Backoff = global::Elarion.Abstractions.Resilience.ResilienceBackoffType.{FormatFrameworkBackoff(retry.Backoff)},");
            if (retry.MaxDelayTicks is { } maxDelayTicks)
                sb.AppendLine(
                    $"                    MaxDelay = global::System.TimeSpan.FromTicks({maxDelayTicks.ToString(CultureInfo.InvariantCulture)}L),");
        ...
        if (policy.Timeout is { } timeout)
            sb.AppendLine(
                $"                Timeout = global::System.TimeSpan.FromTicks({timeout.TimeoutTicks.ToString(CultureInfo.InvariantCulture)}L)");
        else
    ...
    private static string FormatFrameworkBackoff(int backoff) {
        return FormatBackoff(backoff);
    }
