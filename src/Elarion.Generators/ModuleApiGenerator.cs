...
public sealed class ModuleApiGenerator : IIncrementalGenerator {
    ...
    private static void Execute(
        ...
        var registrationsByModule =
            new Dictionary<ModuleScanner.Module, List<(string InterfaceFqn, string ForwarderFqn)>>();

        foreach (var facade in facades.OrderBy(f => f.Namespace, StringComparer.Ordinal)
    ...
    private static List<MemberEntry> SelectMembers(
        ...
        foreach (var handler in handlers
            ...
            if (!usedNames.Add(handler.Name)) {
                spc.ReportDiagnostic(Diagnostic.Create(DuplicateMethod, facade.Location.ToLocation(), facade.Name,
                    handler.Name));
                continue;
            }
    ...
    private static FacadeResult? CreateFacadeResult(GeneratorAttributeSyntaxContext ctx, CancellationToken ct) {
        ...
        if (symbol.ContainingType is not null)
            return new FacadeResult(null,
                ImmutableArray.Create(DiagnosticInfo.Create(FacadeNested, location, symbol.Name)));
        ...
        if (!isPartial)
            return new FacadeResult(null,
                ImmutableArray.Create(DiagnosticInfo.Create(FacadeNotPartial, location, symbol.Name)));
    ...
    private static string GenerateFacade(string ns, FacadeEntry facade, string forwarderName,
        ...
        sb.AppendLine(
            $"internal sealed class {forwarderName}(global::System.IServiceProvider services) : {facade.Name}");
        sb.AppendLine("{");
        foreach (var member in members) {
            sb.AppendLine($"    public {Signature(member)}");
            sb.AppendLine(
                $"        => global::Microsoft.Extensions.DependencyInjection.ServiceProviderServiceExtensions.GetRequiredService<global::Elarion.Abstractions.IHandler<{member.RequestFqn}, {member.ResponseFqn}>>(services).HandleAsync(request, ct);");
        }
