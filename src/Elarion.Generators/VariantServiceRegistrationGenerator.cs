...
public sealed class VariantServiceRegistrationGenerator : IIncrementalGenerator {
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context) {
        var featureVariants = context.SyntaxProvider
            .ForAttributeWithMetadataName(
                ...
                static (ctx, _) => CreateVariantResult(ctx, false))
        ...
        var configurationVariants = context.SyntaxProvider
            .ForAttributeWithMetadataName(
                ...
                static (node, _) => node is ClassDeclarationSyntax,
                static (ctx, _) => CreateVariantResult(ctx, true))
            .Where(static result => result is not null)
            .Select(static (result, _) => result!)
            .Collect()
    ...
    private static VariantResult? CreateVariantResult(GeneratorAttributeSyntaxContext ctx, bool isConfiguration) {
        ...
        foreach (var named in attribute.NamedArguments) {
            if (named.Key == variantPropertyName && named.Value.Value is string v &&
                !string.IsNullOrWhiteSpace(v)) variant = v;

            if (named.Key == "IsDefault" && named.Value.Value is bool d) isDefaultFlag = d;
        }
        ...
        if (isConfiguration && variant is not null) variant = variant.ToLowerInvariant();
        ...
        foreach (var byContract in impls.GroupBy(static i => i.ContractFqn, StringComparer.Ordinal)) {
            ...
            foreach (var impl in ordered)
                if (!string.Equals(impl.SelectorKey, selectorKey, StringComparison.Ordinal)) {
                    spc.ReportDiagnostic(DiagnosticInfo.Create(
                            ConflictingSelector, impl.Location, impl.ContractFqn, selectorKey, impl.SelectorKey)
                        .ToDiagnostic());
                    hasError = true;
                }
            ...
            for (var extra = 1; extra < defaults.Count; extra++) {
                spc.ReportDiagnostic(DiagnosticInfo.Create(
                        DuplicateVariantKey, defaults[extra].Location, "(default)", defaults[extra].ContractFqn)
                    .ToDiagnostic());
                hasError = true;
            }
            ...
            var defaultKeyExpr = defaultImpl is null
                ? "null"
                : defaultImpl.Value is null
                    ? DefaultKeySentinelExpr
                    : FormatLiteral(defaultImpl.Value);

            groups.Add(new ContractGroup(
                ordered[0].ContractFqn,
                selectorKey,
                isConfiguration,
                ordered[0].Namespace,
                HintNames.Sanitize(ordered[0].ContractFqn) + "_VariantService",
                defaultImpl is not null,
                defaultKeyExpr,
                ordered));
        }
    ...
    private static string GeneratePerContractRegistration(ContractGroup group) {
        ...
        sb.AppendLine("{");
        sb.AppendLine(
            $"    public static IServiceCollection {group.RegistrationMethodName}(this IServiceCollection services)");
        sb.AppendLine("    {");
        ...
        sb.AppendLine(
            $"            .{registrationMethod}<{group.ContractFqn}>(services, {FormatLiteral(group.SelectorKey)}, {group.DefaultKeyExpr});");
    ...
    private static void GenerateModuleAggregations(
        ...
        foreach (var group in groups) {
            ...
            if (best is not null) moduleGroups[best].Add(group);
        }
        ...
        foreach (var kvp in moduleGroups.OrderBy(k => k.Key.Name, StringComparer.Ordinal)) {
            ...
            sb.AppendLine("{");
            sb.AppendLine(
                $"    public static IServiceCollection Add{module.Name}VariantServices(this IServiceCollection services)");
            sb.AppendLine("    {");
    ...
    private static string GetTypeReference(ContractGroup group) {
        return group.Namespace.Length == 0
    ...
    private static string GetNamespace(INamedTypeSymbol typeSymbol) {
        return typeSymbol.ContainingNamespace is { IsGlobalNamespace: false } ns ? ns.ToDisplayString() : string.Empty;
    }
    ...
    private static string Sanitize(string value) {
        ...
        foreach (var ch in value.Replace("global::", string.Empty)) sb.Append(char.IsLetterOrDigit(ch) ? ch : '_');
