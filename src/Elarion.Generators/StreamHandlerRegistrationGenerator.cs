...
public sealed class StreamHandlerRegistrationGenerator : IIncrementalGenerator {
    ...
    private const string AuthorizationDefaults = "Elarion.Abstractions.Authorization.ElarionAuthorizationDefaultsAttribute";
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context) {
        var candidates = context.SyntaxProvider.CreateSyntaxProvider(
            ...
            .Collect()
            .Select(static (names, _) => names.Where(static x => x is not null).Select(static x => x!).Distinct(StringComparer.Ordinal).OrderBy(static x => x, StringComparer.Ordinal).ToImmutableArray().ToEquatableArray())
        ...
        context.RegisterSourceOutput(models, static (spc, handlers) => {
            foreach (var handler in handlers) {
                ...
                spc.AddSource($"{HintNames.Sanitize(handler.HandlerFqn)}.stream.g.cs", SourceText.From(Emit(handler), Encoding.UTF8));
            }
        });
    ...
    private static string? Candidate(GeneratorSyntaxContext context, CancellationToken ct) {
        ...
        if (context.SemanticModel.GetDeclaredSymbol(declaration, ct) is not INamedTypeSymbol type || type.IsAbstract || type.IsGenericType)
            return null;
    ...
    private static EquatableArray<StreamInfo> Resolve(EquatableArray<string> names, EquatableArray<ModuleScanner.Module> modules, Compilation compilation, CancellationToken ct) {
        ...
        var validationWalk = compilation.GetTypeByMetadataName(ValidationExtensions) is null
            ? null : new ValidatableTypeWalker.Context(compilation.Assembly);
        ...
        foreach (var name in names) {
            ...
            var resources = classDeclaration is null
                ...
                : HandlerRegistrationGenerator.BuildResourceBindings(
                    ...
                    .Select(static binding => new ResourceBinding(binding.ResourceTypeFqn, binding.Operation, binding.IdPath, binding.ResourceTypeName))
    ...
    private static bool HasAttribute(INamedTypeSymbol type, params string[] names) {
        for (INamedTypeSymbol? current = type; current is not null; current = current.BaseType)
            if (current.GetAttributes().Any(a => a.AttributeClass is not null && names.Contains(a.AttributeClass.ToDisplayString(), StringComparer.Ordinal)))
                return true;
        return false;
    }
    ...
    private static List<AttributeData> EnumerateRequireResources(INamedTypeSymbol type) {
        ...
        for (INamedTypeSymbol? current = type; current is not null; current = current.BaseType)
            resources.AddRange(current.GetAttributes().Where(attribute =>
                attribute.AttributeClass?.ToDisplayString() == "Elarion.Abstractions.Authorization.RequireResourceAttribute"));
        return resources;
    }

    private static bool ResolveAuthDefault(INamedTypeSymbol type, Compilation compilation, EquatableArray<ModuleScanner.Module> modules) {
        ...
        foreach (var module in modules) {
            if (!ModuleScanner.IsInScope(type.ContainingNamespace.ToDisplayString(), module.Namespace) || module.Namespace.Length <= best) continue;
            if (compilation.Assembly.GetTypeByMetadataName(module.MetadataName) is { } symbol) {
                var found = symbol.GetAttributes().FirstOrDefault(a => SymbolEqualityComparer.Default.Equals(a.AttributeClass, attribute));
                if (found is not null) { selected = found; best = module.Namespace.Length; }
            }
        }
        selected ??= compilation.Assembly.GetAttributes().FirstOrDefault(a => SymbolEqualityComparer.Default.Equals(a.AttributeClass, attribute));
        return selected?.NamedArguments.FirstOrDefault(p => p.Key == "RequireAuthenticated").Value.Value as bool? ?? true;
    }
    ...
    private static bool ParseFeatureGates(INamedTypeSymbol type, ImmutableArray<DiagnosticInfo>.Builder diagnostics) {
        ...
        foreach (var gate in gates) {
            ...
            if (HandlerRegistrationGenerator.HasEffectiveFeatureName(gate)) {
                hasEffectiveGate = true;
            }
        }
    ...
    private static INamedTypeSymbol? FindStream(INamedTypeSymbol type) => type.AllInterfaces.FirstOrDefault(i => i.OriginalDefinition.ToDisplayString() == StreamInterface);

    private static List<(string Namespace, AttributeData Attribute)> BuildModuleDecoratorLists(Compilation compilation, EquatableArray<ModuleScanner.Module> modules, CancellationToken ct) {
    ...
    private static AttributeData? ResolveDecoratorList(INamedTypeSymbol type, Compilation compilation, List<(string Namespace, AttributeData Attribute)> modules) {
        var decoratorList = compilation.GetTypeByMetadataName(DecoratorListMetadata);
    ...
    private static AttributeData? FindPipelineDecoratorList(ImmutableArray<AttributeData> attributes, INamedTypeSymbol decoratorList) {
        foreach (var attribute in attributes)
            if (attribute.AttributeClass is not null)
                foreach (var meta in attribute.AttributeClass.GetAttributes())
                    if (SymbolEqualityComparer.Default.Equals(meta.AttributeClass, decoratorList)) return meta;
        return null;
    }

    private static EquatableArray<Decorator> ParseDecorators(INamedTypeSymbol handler, ITypeSymbol request, ITypeSymbol item, Compilation compilation, List<(string Namespace, AttributeData Attribute)> modules, SymbolDisplayFormat format, ImmutableArray<DiagnosticInfo>.Builder diagnostics) {
        var list = ResolveDecoratorList(handler, compilation, modules);
        var result = ImmutableArray.CreateBuilder<Decorator>();
        if (list?.ConstructorArguments.FirstOrDefault().Values is not { } values) return result.ToImmutable().ToEquatableArray();
        foreach (var value in values) {
            ...
            try { closed = definition.Construct(request, item); } catch (ArgumentException) { continue; }
            var ctor = closed.InstanceConstructors
                .Where(c => c.DeclaredAccessibility == Accessibility.Public && c.Parameters.Length > 0 && IsStream(c.Parameters[0].Type))
                .OrderByDescending(c => c.Parameters.Length).FirstOrDefault();
            if (ctor is null || !closed.AllInterfaces.Any(i => i.OriginalDefinition.ToDisplayString() == StreamInterface)) continue;
            ...
            for (var i = 1; i < ctor.Parameters.Length; i++) {
                ...
                dependencies.Add(new Dependency(dependency, ctor.Parameters[i].Type.ToDisplayString() == StreamMetadata));
            }
            ...
            if (predicate == DecoratorPredicate.Result.NotPublic) {
                diagnostics.Add(DiagnosticInfo.Create(HandlerRegistrationGenerator.NonPublicAppliesToPredicate, location, definition.Name, "StreamHandlerMetadata"));
                continue;
            }
            if (predicate == DecoratorPredicate.Result.UnsupportedSignature) {
                diagnostics.Add(DiagnosticInfo.Create(HandlerRegistrationGenerator.UnsupportedAppliesToSignature, location, definition.Name, "StreamHandlerMetadata"));
                continue;
            }
            result.Add(new Decorator(closed.ToDisplayString(format), OpenName(closed.ToDisplayString(format)), dependencies.ToImmutable().ToEquatableArray(), predicate == DecoratorPredicate.Result.Conditional));
        }
        return result.ToImmutable().ToEquatableArray();
    }

    private static bool IsStream(ITypeSymbol type) => type is INamedTypeSymbol named && named.OriginalDefinition.ToDisplayString() == StreamInterface;
    private static string OpenName(string closed) => closed.Substring(0, closed.IndexOf('<')) + "<,>";

    private static bool SatisfiesConstraints(INamedTypeSymbol definition, ITypeSymbol request, ITypeSymbol item, Compilation compilation) {
        ...
        return Satisfies(definition.TypeParameters[0], request, substitutions, compilation) && Satisfies(definition.TypeParameters[1], item, substitutions, compilation);
    }

    private static bool Satisfies(ITypeParameterSymbol parameter, ITypeSymbol argument, IReadOnlyDictionary<ITypeParameterSymbol, ITypeSymbol> substitutions, Compilation compilation) {
        ...
        foreach (var constraint in parameter.ConstraintTypes) {
            if (ResolveConstraint(constraint, substitutions, compilation) is not { } resolved || !SatisfiesType(argument, resolved)) return false;
        }
        return true;
    }

    private static ITypeSymbol? ResolveConstraint(ITypeSymbol constraint, IReadOnlyDictionary<ITypeParameterSymbol, ITypeSymbol> substitutions, Compilation compilation) {
        ...
        if (constraint is IArrayTypeSymbol array && ResolveConstraint(array.ElementType, substitutions, compilation) is { } element)
    ...
    private static bool SatisfiesType(ITypeSymbol argument, ITypeSymbol constraint) {
        if (constraint.TypeKind == TypeKind.Interface)
            return SymbolEqualityComparer.Default.Equals(argument, constraint) || argument.AllInterfaces.Any(i => SymbolEqualityComparer.Default.Equals(i, constraint));
        for (ITypeSymbol? current = argument; current is not null; current = current.BaseType)
            if (SymbolEqualityComparer.Default.Equals(current, constraint)) return true;
        return false;
    }
    ...
    private static bool HasPublicParameterlessConstructor(ITypeSymbol type) {
        ...
        return type is INamedTypeSymbol { IsAbstract: false } named && named.InstanceConstructors.Any(ctor => ctor.DeclaredAccessibility == Accessibility.Public && ctor.Parameters.Length == 0);
    }
    ...
    private static string Emit(StreamInfo handler) {
        ...
        sb.AppendLine("// <auto-generated/>").AppendLine("#nullable enable").AppendLine("using Microsoft.Extensions.DependencyInjection;").AppendLine();
        sb.AppendLine($"namespace {handler.Namespace};").AppendLine();
        sb.AppendLine($"public static class {handler.Name}StreamRegistration {{");
        sb.AppendLine("    private static volatile global::System.Collections.Generic.IReadOnlyList<global::Elarion.Abstractions.Pipeline.PipelineStep>? __pipeline;");
        sb.AppendLine("    private static readonly global::Elarion.Abstractions.Pipeline.StreamHandlerMetadata __metadata =");
        sb.AppendLine($"        new(typeof({handler.HandlerFqn}), typeof({handler.RequestFqn}), typeof({handler.ItemFqn}), static () => __pipeline ?? global::System.Array.Empty<global::Elarion.Abstractions.Pipeline.PipelineStep>());");
        for (var i = 0; i < handler.Decorators.Count; i++) if (handler.Decorators[i].Conditional) sb.AppendLine($"    private static readonly bool __applies{i} = {handler.Decorators[i].Fqn}.AppliesTo(__metadata);");
        sb.AppendLine($"    public static IServiceCollection Add{handler.Name}Stream(this IServiceCollection services, ServiceLifetime lifetime = ServiceLifetime.Scoped) {{");
        AppendConcreteRegistration(sb, handler.HandlerFqn);
        sb.AppendLine($"        services.Add(new ServiceDescriptor(typeof({iface}), sp => Build(sp), __handlerLifetime));");
        ...
        sb.AppendLine("        var steps = __pipeline is null ? new global::System.Collections.Generic.List<global::Elarion.Abstractions.Pipeline.PipelineStep>() : null;");
        for (var i = handler.Decorators.Count - 1; i >= 0; i--) {
            ...
            foreach (var dep in d.Dependencies) sb.Append(dep.IsMetadata ? ", __metadata" : $", sp.GetRequiredService<{dep.Fqn}>()");
            sb.AppendLine(");");
            sb.AppendLine($"        steps?.Add(new global::Elarion.Abstractions.Pipeline.PipelineStep(typeof({d.OpenFqn}), {d.Conditional.ToString().ToLowerInvariant()}));");
            if (d.Conditional) sb.AppendLine("        }");
        }
        if (handler.HasValidation) { sb.AppendLine($"        handler = new global::Elarion.Pipeline.StreamValidationDecorator<{handler.RequestFqn}, {handler.ItemFqn}>(handler, sp.GetRequiredService<global::Elarion.Abstractions.Validation.IRequestValidator>());"); sb.AppendLine("        steps?.Add(new(typeof(global::Elarion.Pipeline.StreamValidationDecorator<,>), false));"); }
        if (handler.HasFeature) { sb.AppendLine($"        handler = new global::Elarion.Pipeline.StreamFeatureGateDecorator<{handler.RequestFqn}, {handler.ItemFqn}>(handler, __metadata, sp.GetRequiredService<global::Elarion.Abstractions.Features.IFeatureFlagService>());"); sb.AppendLine("        steps?.Add(new(typeof(global::Elarion.Pipeline.StreamFeatureGateDecorator<,>), false));"); }
        if (handler.HasAuthorization) { sb.AppendLine($"        handler = new global::Elarion.Pipeline.StreamAuthorizationDecorator<{handler.RequestFqn}, {handler.ItemFqn}>(handler, __metadata, sp.GetRequiredService<global::Elarion.Abstractions.Authorization.IAuthorizer>(), {handler.RequireAuthenticatedByDefault.ToString().ToLowerInvariant()}, new global::Elarion.Abstractions.Authorization.ResourceRequirementBinding<{handler.RequestFqn}>[] {{"); foreach (var resource in handler.Resources) sb.AppendLine($"            new(typeof({resource.ResourceTypeFqn}), new global::Elarion.Abstractions.Authorization.ResourceOperation({HandlerRegistrationGenerator.FormatStringLiteral(resource.Operation)}), static request => request.{resource.IdPath}, {(resource.ResourceTypeName is null ? "null" : HandlerRegistrationGenerator.FormatStringLiteral(resource.ResourceTypeName))}),"); sb.AppendLine("        });"); sb.AppendLine("        steps?.Add(new(typeof(global::Elarion.Pipeline.StreamAuthorizationDecorator<,>), false));"); }
        sb.AppendLine($"        handler = new global::Elarion.Pipeline.StreamObservabilityDecorator<{handler.RequestFqn}, {handler.ItemFqn}>(handler, \"{handler.Name}\", __metadata, sp.GetServices<global::Elarion.Abstractions.Diagnostics.IHandlerContextEnricher>(), sp.GetService<global::Microsoft.Extensions.Logging.ILoggerFactory>());");
        sb.AppendLine("        steps?.Add(new(typeof(global::Elarion.Pipeline.StreamObservabilityDecorator<,>), false));");
    ...
    private static void AppendConcreteRegistration(StringBuilder sb, string handlerFqn) {
        ...
        sb.AppendLine($"            if (descriptor.IsKeyedService || descriptor.ServiceType != typeof({handlerFqn})) continue;");
        sb.AppendLine("            __handlerLifetime = descriptor.Lifetime;");
        sb.AppendLine("            __hasConcreteRegistration = true;");
        sb.AppendLine("            break;");
        sb.AppendLine("        }");
        sb.AppendLine($"        if (!__hasConcreteRegistration) services.Add(new ServiceDescriptor(typeof({handlerFqn}), typeof({handlerFqn}), __handlerLifetime));");
    }

    private static void EmitModuleAggregations(SourceProductionContext spc, EquatableArray<StreamInfo> handlers, EquatableArray<ModuleScanner.Module> modules) {
        foreach (var module in modules.OrderBy(static m => m.Name, StringComparer.Ordinal)) {
            ...
            sb.AppendLine($"    public static IServiceCollection Add{module.Name}StreamHandlers(this IServiceCollection services, ServiceLifetime lifetime = ServiceLifetime.Scoped) {{");
            foreach (var entry in entries) sb.AppendLine($"        {entry.Namespace}.{entry.Name}StreamRegistration.Add{entry.Name}Stream(services, lifetime);");
            sb.AppendLine("        return services; }\n}");
            spc.AddSource($"{HintNames.Sanitize(module.Namespace + "." + module.Name)}.StreamHandlerExtensions.g.cs", SourceText.From(sb.ToString(), Encoding.UTF8));
            var prefix = module.Namespace.Length == 0 ? "global::" : $"global::{module.Namespace}.";
            ModuleDefaultsEmitter.EmitFiller(spc, module.Namespace, module.TypeName, ModuleDefaultsEmitter.AddStreamHandlersMethod, "StreamHandlers", $"{prefix}{module.Name}StreamHandlerExtensions.Add{module.Name}StreamHandlers(services);");
        }
    }

    private sealed record StreamInfo(string HandlerFqn, string Name, string Namespace, string RequestFqn, string ItemFqn, EquatableArray<Decorator> Decorators, bool HasAuthorization, bool RequireAuthenticatedByDefault, EquatableArray<ResourceBinding> Resources, bool HasFeature, bool HasValidation, EquatableArray<DiagnosticInfo> Diagnostics);
    private sealed record Decorator(string Fqn, string OpenFqn, EquatableArray<Dependency> Dependencies, bool Conditional);
    private sealed record Dependency(string Fqn, bool IsMetadata);
    private sealed record ResourceBinding(string ResourceTypeFqn, string Operation, string IdPath, string? ResourceTypeName);
}
