...
namespace Elarion.Scheduling.EntityFrameworkCore.Generators;

/// <summary>
/// Fills every partial <c>DbContext</c> annotated with <c>[GenerateElarionSchedulerClaims]</c> with the
/// <c>DbSet&lt;SchedulerClaimEntity&gt;</c> property and an implementation of the EF generator's per-feature
/// model-configuration seam that calls <c>UseElarionSchedulerClaims</c>. So the host writes neither the DbSet
/// nor the table mapping — mirroring <c>[GenerateElarionIdempotencyKeys]</c>.
/// </summary>
[Generator(LanguageNames.CSharp)]
public sealed class ElarionSchedulerClaimsGenerator : IIncrementalGenerator {
    private const string AttributeMetadataName = "Elarion.Scheduling.EntityFrameworkCore.GenerateElarionSchedulerClaimsAttribute";
    private const string GenerateDbSetsAttributeName = ElarionGeneratorConventions.GenerateDbSetsAttribute;
    private const string SchedulingNamespace = "global::Elarion.Scheduling.EntityFrameworkCore";
    ...
    private static readonly string SeamMethodName =
        ElarionGeneratorConventions.ModelConfigurationSeamName("GenerateElarionSchedulerClaimsAttribute");

    private static readonly DiagnosticDescriptor MissingGenerateDbSets = new(
        "ELSCH001",
        "[GenerateElarionSchedulerClaims] requires [GenerateDbSets]",
        "Context '{0}' is annotated with [GenerateElarionSchedulerClaims] but not [GenerateDbSets]; add [GenerateDbSets] "
        + "so the settings DbSet and model-configuration seam are generated",
        "Elarion.Scheduling.EntityFrameworkCore",
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context) {
        var targets = context.SyntaxProvider
            ...
            .WithTrackingName("SchedulerClaimsTargets");
    ...
    private sealed record SchedulerClaimsTarget(
        string Namespace,
        string ContextName,
        string HintName,
        bool SnakeCase,
    ...
    private static SchedulerClaimsTarget? GetTarget(GeneratorAttributeSyntaxContext ctx) {
        ...
        return new SchedulerClaimsTarget(
            ns,
            contextName,
            hintName,
            snakeCase,
    ...
    private static void Emit(SourceProductionContext spc, SchedulerClaimsTarget target) {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine("// Source: Elarion.Scheduling.EntityFrameworkCore.Generators.ElarionSchedulerClaimsGenerator");
        ...
        sb.AppendLine("{");
        sb.AppendLine($"    public DbSet<{SchedulingNamespace}.SchedulerClaimEntity> SchedulerClaims => Set<{SchedulingNamespace}.SchedulerClaimEntity>();");
        sb.AppendLine();
        ...
        sb.AppendLine($"        {SchedulingNamespace}.SchedulerModelBuilderExtensions.UseElarionSchedulerClaims(");
        sb.AppendLine(
        ...
        spc.AddSource($"{target.HintName}.ElarionSchedulerClaims.g.cs", SourceText.From(sb.ToString(), Encoding.UTF8));
    }
}
