...
namespace Elarion.Coordination.PostgreSql.Generators;

/// <summary>
/// Fills every partial <c>DbContext</c> annotated with <c>[GenerateElarionRoleLeases]</c> with the
/// <c>DbSet&lt;RoleLeaseEntity&gt;</c> property and an implementation of the EF generator's per-feature
/// model-configuration seam that calls <c>UseElarionRoleLeases</c> — the lease table behind role
/// leases (ADR-0049). Mirrors <c>[GenerateElarionActorSnapshots]</c>.
/// </summary>
[Generator(LanguageNames.CSharp)]
public sealed class ElarionRoleLeasesGenerator : IIncrementalGenerator {
    private const string AttributeMetadataName = "Elarion.Coordination.PostgreSql.GenerateElarionRoleLeasesAttribute";
    private const string GenerateDbSetsAttributeName = ElarionGeneratorConventions.GenerateDbSetsAttribute;
    private const string CoordinationNamespace = "global::Elarion.Coordination.PostgreSql";
    ...
    private static readonly string SeamMethodName =
        ElarionGeneratorConventions.ModelConfigurationSeamName("GenerateElarionRoleLeasesAttribute");

    private static readonly DiagnosticDescriptor MissingGenerateDbSets = new(
        "ELROLE001",
        "[GenerateElarionRoleLeases] requires [GenerateDbSets]",
        "Context '{0}' is annotated with [GenerateElarionRoleLeases] but not [GenerateDbSets]; add [GenerateDbSets] "
    ...
    private sealed record RoleLeasesTarget(
        string Namespace,
        string ContextName,
        string HintName,
        bool SnakeCase,
    ...
    private static void Emit(SourceProductionContext spc, RoleLeasesTarget target) {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine("// Source: Elarion.Coordination.PostgreSql.Generators.ElarionRoleLeasesGenerator");
        ...
        spc.AddSource($"{target.HintName}.ElarionRoleLeases.g.cs", SourceText.From(sb.ToString(), Encoding.UTF8));
    }
}
