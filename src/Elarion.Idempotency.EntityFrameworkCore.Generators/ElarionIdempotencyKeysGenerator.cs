...
public sealed class ElarionIdempotencyKeysGenerator : IIncrementalGenerator {
    private const string AttributeMetadataName =
        "Elarion.Idempotency.EntityFrameworkCore.GenerateElarionIdempotencyKeysAttribute";
    ...
    private static void Emit(SourceProductionContext spc, IdempotencyTarget target) {
        ...
        sb.AppendLine("{");
        sb.AppendLine(
            $"    public DbSet<{IdempotencyNamespace}.IdempotencyKeyEntity> IdempotencyKeys => Set<{IdempotencyNamespace}.IdempotencyKeyEntity>();");
        sb.AppendLine();
        sb.AppendLine(
            "    // Implements the per-feature model-configuration seam the EF DbContext generator calls at the");
        sb.AppendLine(
            "    // end of ConfigureEntities, so it composes with [GenerateElarionIdentity]/[GenerateElarionResourceGrants].");
