...
| [`Elarion.Devices`](https://github.com/swimmesberger/Elarion/tree/main/src/Elarion.Devices) | Device identity and provisioning for gateways: single-use CSPRNG pairing codes redeemed for a per-device symmetric key, the `IDeviceKeyStore`/`IPairingCodeStore` seams, a constant-time HMAC challenge verifier shaped for connection-handshake authenticators, and the device `ClaimsPrincipal` factory (ADR-0054). |
| [`Elarion.Devices.EntityFrameworkCore`](https://github.com/swimmesberger/Elarion/tree/main/src/Elarion.Devices.EntityFrameworkCore) | Durable EF-backed device identity stores: device key and pairing-code tables mapped via `[GenerateElarionDeviceIdentity]`, with atomic single-winner pairing-code claims (`AddElarionDeviceIdentityEntityFrameworkCore<TDbContext>()`). |
...
| [`Elarion.Migrations`](https://github.com/swimmesberger/Elarion/tree/main/src/Elarion.Migrations) | Database-neutral SQL migration engine for EF-free (NativeAOT) hosts: embedded `V__`/`R__` scripts, normalized SHA-256 checksums, roll-forward history rows committing with their script — no repair command, no undo (ADR-0057/ADR-0060). Drives a provider through the `IMigrationDatabase` seam; contains no driver. EF applications keep EF migrations. |
| [`Elarion.Migrations.PostgreSql`](https://github.com/swimmesberger/Elarion/tree/main/src/Elarion.Migrations.PostgreSql) | PostgreSQL provider for `Elarion.Migrations`: one dedicated connection under a session-level `pg_advisory_lock` — the multi-node PostgreSQL AOT tier. |
| [`Elarion.Migrations.Sqlite`](https://github.com/swimmesberger/Elarion/tree/main/src/Elarion.Migrations.Sqlite) | SQLite provider for `Elarion.Migrations`: a per-file in-process lock — the single-node / edge AOT tier (one file per node). |
| [`Elarion.Sql`](https://github.com/swimmesberger/Elarion/tree/main/src/Elarion.Sql) | AOT-native SQL row mapping for EF-free (NativeAOT) hosts: `[SqlRecord]` generates an explicit `ISqlRowMapper<T>` (ordinal-cached typed reads, typed parameter binding, table/column constants) plus injection-safe SQL interpolation — no reflection, no interception, unmapped types fail at compile time (ADR-0058). EF applications keep EF Core. |
...
| [`Elarion.Connections`](https://github.com/swimmesberger/Elarion/tree/main/src/Elarion.Connections) | Bidirectional client-connection kernel: the node-local connection registry with lifecycle observers, the transport-neutral codec seam, and the bridge that serves client-event subscriptions over any connection adapter (ADR-0053). |
| [`Elarion.Connections.AspNetCore`](https://github.com/swimmesberger/Elarion/tree/main/src/Elarion.Connections.AspNetCore) | WebSocket connection adapter shaped for device gateways: `MapElarionConnectionSocket<THandler>` owns accept, reassembly, and lifecycle; the app supplies only the authenticator and the codec. |
| [`Elarion.Connections.Tcp`](https://github.com/swimmesberger/Elarion/tree/main/src/Elarion.Connections.Tcp) | Raw TCP connection adapter for proprietary device protocols: hosted listener and dial-out with jittered reconnect over the same handler/codec seams, with adapter-owned message framing (length-prefixed and delimited built-ins). |
| [`Elarion.Connections.Simulation`](https://github.com/swimmesberger/Elarion/tree/main/src/Elarion.Connections.Simulation) | Simulation and test utilities for connections: in-memory sink doubles, awaitable lifecycle observers, a framed simulator client, and the socket-free in-memory TCP link running the full connection lifecycle. |
...
| [`Elarion.EntityFrameworkCore.BulkOperations`](https://github.com/swimmesberger/Elarion/tree/main/src/Elarion.EntityFrameworkCore.BulkOperations) | Provider-neutral bulk operations: non-tracking `ExecuteInsertAsync` over `DbSet<T>` (streaming `IAsyncEnumerable` overload, joins the ambient transaction) with the `IBulkInsertProvider` seam — aligned with the dotnet/efcore bulk-import design sketch. |
| [`Elarion.BulkOperations.PostgreSql`](https://github.com/swimmesberger/Elarion/tree/main/src/Elarion.BulkOperations.PostgreSql) | PostgreSQL bulk-insert provider over binary `COPY` — `options.UseNpgsql(…).UseElarionPostgreSqlBulkOperations()`; benchmarked at raw `NpgsqlBinaryImporter` parity, ~12× `SaveChanges` at 100k rows. |
