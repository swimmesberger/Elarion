...
| [`Elarion.Migrations.PostgreSql`](https://github.com/swimmesberger/Elarion/tree/main/src/Elarion.Migrations.PostgreSql) | SQL migration runner for EF-free (NativeAOT) hosts: embedded `V__`/`R__` scripts, normalized SHA-256 checksums, a session-level advisory lock, history rows committing with their script — no repair command, no undo, PostgreSQL only (ADR-0057). EF applications keep EF migrations. |
