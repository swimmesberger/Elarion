...
public class SqlStatementBuildingBenchmarks {
    ...
    // The generated column-list / table constants (current shape: two separate raw-spliced consts).
    private const string ColumnsAll = "device_id, metric, recorded_at, value, meta";
    ...
    // so ONE {Row.Select} hole replaces the two {Columns.All:raw} + {TableName:raw} holes.
    ...
    // BEFORE — the real GetLatestReading shape: two raw-spliced consts + two value holes.
    [Benchmark(Baseline = true)]
    public int Current_TwoRawSplices() {
        var sql = new SqlStatement(
            $"SELECT {ColumnsAll:raw} FROM {TableName:raw} WHERE device_id = {_id} AND metric = {_metric} ORDER BY recorded_at DESC LIMIT 1");
        return sql.Text.Length + sql.ParameterValues.Count;
    }

    // AFTER — one cached fragment splice + two value holes. Must be <= the two-raw-splice path.
    [Benchmark]
    public int Redesign_OneFragmentSplice() {
        var sql = new SqlStatement(
