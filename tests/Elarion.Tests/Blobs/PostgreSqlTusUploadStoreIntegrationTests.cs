...
public sealed class PostgreSqlTusUploadStoreIntegrationTests(PostgreSqlTusFixture fixture)
    ...
    public async Task CreateAppendComplete_ProducesPendingBlobWithContent() {
        ...
        var completed = await store.AppendAsync(created.Id, 0, new MemoryStream(payload), payload.Length, ct);
    ...
    public async Task ResumableAppend_TwoChunks_Completes() {
        ...
        var afterFirst = await store.AppendAsync(created.Id, 0, new MemoryStream([1, 2, 3]), 3, ct);
        afterFirst.Offset.Should().Be(3);
        afterFirst.BlobRef.Should().BeNull();

        (await store.GetAsync(created.Id, ct))!.Offset.Should().Be(3);

        var completed = await store.AppendAsync(created.Id, 3, new MemoryStream([4, 5, 6]), 3, ct);
        completed.BlobRef.Should().NotBeNull();
    ...
    public async Task AppendAsync_ZeroLength_FinalizesToEmptyBlob() {
        ...
        var completed = await store.AppendAsync(created.Id, 0, Stream.Null, 0, ct);
    ...
    public async Task AppendAsync_WrongOffset_Throws() {
        ...
        var act = async () => await store.AppendAsync(created.Id, 2, new MemoryStream([1, 2]), 2, ct);
    ...
    public async Task DeleteExpiredAsync_RemovesIncompleteOnly() {
        ...
        var completed = await store.AppendAsync(willComplete.Id, 0, new MemoryStream([1, 2]), 2, ct);
        completed.BlobRef.Should().NotBeNull();
    ...
    private static PostgreSqlTusUploadStore<TusBlobDbContext> CreateStore(
        ...
        return new PostgreSqlTusUploadStore<TusBlobDbContext>(context, blobStore, options, TimeProvider.System);
    }
