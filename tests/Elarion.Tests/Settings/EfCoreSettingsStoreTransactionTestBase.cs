...
/// <summary>
/// Provider-neutral contract verifying that <see cref="EfCoreSettingsStore{TDbContext}"/> writes enlist in
/// the caller's ambient EF Core transaction: an insert/update/remove issued on a context that already holds
/// an open transaction commits or rolls back together with that transaction. A derived class per provider
/// binds the matching <see cref="ISettingsStoreFixture"/>.
/// </summary>
public abstract class EfCoreSettingsStoreTransactionTestBase(ISettingsStoreFixture fixture) {
