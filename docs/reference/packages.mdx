...
| `Elarion.Grpc` | A gRPC service override needs explicit protobuf/application mapping, a correctly seeded handler dispatch scope, and stable `AppError` → `RpcException` translation without taking an ASP.NET gRPC hosting dependency. |
