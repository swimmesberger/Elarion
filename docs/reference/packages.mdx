...
| `Elarion.Grpc` | A gRPC service override needs an injected typed invoker with explicit protobuf/application mapping, one host-configured principal source, a correctly seeded dispatch scope, and stable `AppError` → `RpcException` translation without an ASP.NET gRPC hosting dependency. |
| `Elarion.Grpc.AspNetCore` | A grpc-dotnet ASP.NET Core host wants the conventional `AddGrpc().AddElarion()` setup and `ServerCallContext.InvokeElarionAsync(...)`, with `HttpContext.User` and `RequestServices` adopted automatically. This is the recommended gRPC host package. |
