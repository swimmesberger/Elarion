...
AOT host pairs `Elarion.Sql` with the database-neutral `Elarion.Migrations` engine and one provider that
picks the database for both — on PostgreSQL that provider is `Elarion.Sql.PostgreSql`, whose single
`AddElarionPostgreSql(connectionString)` registers one `NpgsqlDataSource` shared by the access tier and
migrations. The registrations that follow (`AddElarionSqlUnitOfWork`, `AddElarionMigrations`) are neutral.
...
| `Elarion.Migrations` | An EF-free NativeAOT host needs embedded versioned/repeatable SQL discovery, checksums, planning, and roll-forward execution without a database driver — the provider-neutral `AddElarionMigrations` over a provider registered elsewhere (`Elarion.Sql.PostgreSql`, `Elarion.Migrations.Sqlite`). |
| `Elarion.Migrations.Sqlite` | The migration engine should run on a single-node or edge SQLite file with per-file process coordination. `AddElarionSqlite(connectionString)` picks the provider; the neutral `AddElarionMigrations` runs the schema — the same shape as PostgreSQL. (SQLite is migration-only; there is no SQL access tier for it.) |
...
| EF-free NativeAOT host (SQLite, migrations only) | `Elarion.Migrations` + `Elarion.Migrations.Sqlite` |
