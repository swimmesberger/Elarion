...
| `Elarion.Actors.PostgreSql` | Actor snapshots need PostgreSQL durability/query access, single-homed actors need the `"actors"` role binding, or virtual-sharded actors need fixed `actors:shard-N` role leases via `AddElarionPostgreSqlActorSharding<TDbContext>()`. |
