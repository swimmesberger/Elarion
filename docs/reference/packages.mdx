...
AOT hosts pair `Elarion.Sql` with the database-neutral `Elarion.Migrations` engine and exactly one provider.
...
| `Elarion.Migrations` | An EF-free NativeAOT host needs embedded versioned/repeatable SQL discovery, checksums, planning, and roll-forward execution without a database driver. |
| `Elarion.Migrations.PostgreSql` | The migration engine should run on PostgreSQL with a session advisory lock across roughly 1–10 nodes. |
| `Elarion.Migrations.Sqlite` | The migration engine should run on a single-node or edge SQLite file with per-file process coordination. |
...
| EF-free NativeAOT host | `Elarion.Sql` + `Elarion.Migrations` + `Elarion.Migrations.PostgreSql` or `Elarion.Migrations.Sqlite` |
