...
| Domain / entity library (if separate) | `Elarion.EntityFrameworkCore` — required wherever `[EntityConfiguration]`/`[GenerateDbSets]` types are declared, since its generator emits the per-assembly configuration manifest. |
