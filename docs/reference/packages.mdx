...
| `Elarion.Grpc` | A custom gRPC host needs typed application-request invocation, optional inline mapper delegates, one host-configured principal source, a correctly seeded dispatch scope, and stable `AppError` → `RpcException` translation without an ASP.NET dependency. |
| `Elarion.Grpc.AspNetCore` | A grpc-dotnet ASP.NET Core host wants `AddGrpc().AddElarion()` and `ServerCallContext.InvokeElarionAsync<TRequest,TResponse>(request)`, with `HttpContext.User` and `RequestServices` adopted automatically. This is the recommended gRPC host package. |
