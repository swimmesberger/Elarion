...
| Package | Add it when |
| --- | --- |
| `Elarion.EntityFrameworkCore` | EF models need generated `DbSet`s, entity configuration application, key conventions, keyset paging support, or the opt-in value-shape conventions (`UseElarionEnumStringConversions`, `HasElarionJsonStringArray`, `ElarionValueComparers`). Depends on `Microsoft.EntityFrameworkCore` (provider-neutral) for those conventions. |
