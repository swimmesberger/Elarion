...
AOT hosts pair `Elarion.Sql` (with its `Elarion.Sql.PostgreSql` provider) and the database-neutral
`Elarion.Migrations` engine with exactly one migration provider — a single `NpgsqlDataSource` backs both.
...
| `Elarion.Sql` | An EF-free NativeAOT host needs generated row mappers and injection-safe SQL interpolation with no reflection fallback. Also supplies the EF-free `IUnitOfWork` — `AddElarionSqlUnitOfWork()` registers a scoped `ISqlSession` so handler transactions commit atomically without EF. Stays Npgsql-free; the data source is the `IElarionSqlDataSourceProvider` seam. |
| `Elarion.Sql.PostgreSql` | The EF-free SQL tier should run on PostgreSQL: `AddElarionPostgreSqlDataSource(connectionString)` registers one central `NpgsqlDataSource` (the shared core, EF Core's `DbContext` analogue) plus the provider the session opens from, and `Elarion.Migrations.PostgreSql` borrows the same source so the database is configured once. |
...
| EF-free NativeAOT host | `Elarion.Sql` + `Elarion.Sql.PostgreSql` + `Elarion.Migrations` + `Elarion.Migrations.PostgreSql` or `Elarion.Migrations.Sqlite` |
