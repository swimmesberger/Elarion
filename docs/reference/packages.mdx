...
| Package | Add it when |
| --- | --- |
| `Elarion.EntityFrameworkCore` | EF models need generated `DbSet`s, entity configuration application, key conventions, keyset paging support, ambient tenant scoping (`[GenerateElarionTenantScoping]`, `ITenantScoped<T>`), or the opt-in value-shape conventions (`UseElarionEnumStringConversions`, `HasElarionJsonStringArray`, `ElarionValueComparers`). Depends on `Microsoft.EntityFrameworkCore` (provider-neutral) for those conventions and on `Elarion.Abstractions` for the tenancy contracts. |
...
| `Elarion.Paging` | EF queries need keyset/offset execution, opaque cursors, or sort maps. (`[ResourceFilter]` and `WhereAuthorized` moved to `Elarion.Abstractions`, so data-level authorization no longer pulls this in.) |
...
| `Elarion.WebPush` | Users should be notified while the app is closed: Web Push (VAPID) delivery to their browsers/PWAs with dead-subscription cleanup, a subscription-store seam, and an in-memory default. |
| `Elarion.WebPush.EntityFrameworkCore` | Push subscriptions and the VAPID key pair need durable EF/PostgreSQL tables. |
| `Elarion.WebPush.AspNetCore` | The browser package's default transport should call `MapElarionWebPush` endpoints rather than application handlers. |

See [events and messaging](/docs/capabilities/events) for domain events, integration events, outbox,
client events, and ordered streams, and [Web Push](/docs/capabilities/web-push) for notifying users
whose app is closed.
...
| `Elarion.Settings.EntityFrameworkCore` | Settings need durable relational storage with optimistic concurrency (provider-neutral; PostgreSQL and SQLite are exercised by the test suite). |
...
| `@swimmesberger/elarion-webpush` | A browser/PWA should subscribe to Web Push: availability detection (including the iOS Home Screen requirement), gesture-safe permission, subscribe/refresh helpers, and the service-worker handlers. |

## Dependency rules

- `Elarion.Abstractions` contains implementation-neutral contracts; `Elarion` supplies dependency-light
  runtime behavior and bundles the core generator (`Elarion.Generators`); provider packages such as
  `Elarion.EntityFrameworkCore` and `Elarion.Sql` bundle their own generators.
...
| Frontend | `@swimmesberger/elarion-jsonrpc-client-generator`; optionally `@swimmesberger/elarion-contributions` and `@swimmesberger/elarion-webpush` |
