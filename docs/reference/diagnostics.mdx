...
Inbox attachment for handler-form integration-event consumers (on by default; `[Inbox]` opts out or tunes
retention). See [Handling duplicates](/docs/capabilities/events/consuming-events#handling-duplicates).

| Id | Severity | Meaning | Fix |
| --- | --- | --- | --- |
| ELINBX001 | Warning | A handler declares `[Inbox]` but its request type is not an `IIntegrationEvent`; the inbox only applies to handler-form integration-event consumers (domain events are exactly-once by atomicity), so the attribute has no effect. | Remove `[Inbox]`, or make the handler consume an `IIntegrationEvent`. |
| ELINBX002 | Error | A handler's `[Inbox]` declares a non-positive `RetentionHours`. | Set a positive `RetentionHours` on `[Inbox]` (and keep it above the delivery tier's maximum retry window). |
