...
| ELACT014 | Error | A virtual-sharded actor declares `[ConsumeEvent]`, but the outbox does not route deliveries by virtual shard. | Use shard-aware ingress for the actor, or use `SingleHome` with the existing lease-gated outbox delivery topology. |
