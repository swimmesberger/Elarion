...
| ELHTTP006 | Warning | An `[HttpEndpoint]` handler declares a `CustomizeEndpoint` method that is not `public static void CustomizeEndpoint(IEndpointConventionBuilder)`, so the per-endpoint convention hook is ignored. | Match the hook contract exactly: `public static void CustomizeEndpoint(IEndpointConventionBuilder)` — public, static, void, one parameter. |
...
| ELTEN001 | Error | `Elarion.EntityFrameworkCore` | A context is annotated with `[GenerateElarionTenantScoping]` but not `[GenerateDbSets]`, so the model-configuration seam that applies the tenant filter is not generated. | Add `[GenerateDbSets]` to the context alongside `[GenerateElarionTenantScoping]`. |
...
[Command vs. query](/docs/concepts/handlers#self-typed-markers-declaring-the-response-type).
...
## ELID — Identifier minting

The `Guid.CreateVersion7()` convention (ADR-0038), enforced by an analyzer rather than left to review. Category
`Elarion.Identifiers` — this is about how a `Guid` value is minted, and is unrelated to the user-identity
diagnostics below (`ELIDN`). See [Entity identity](/docs/capabilities/entity-framework#entity-identity).

| Id | Severity | Meaning | Fix |
| --- | --- | --- | --- |
| ELID001 | Warning | A reference to `Guid.NewGuid()` — called, or passed as a method group such as `AddSingleton<Func<Guid>>(Guid.NewGuid)` — mints a random v4 id. **Every** site is flagged deliberately: whether a given id becomes a primary key is not decidable at the call site, and a v7's time-ordered prefix keeps b-tree inserts append-mostly where v4 scatters across the index. | Use `Guid.CreateVersion7()`. If the id must be *unpredictable* (a token or capability code — a v7 leaks its creation instant and carries only 74 random bits), keep v4 and suppress `ELID001` at that site with a justification. There is no code fix: the replacement is a one-token edit named in the message. |
...
## ELWP — Web Push (EF Core)

Web Push table generation for `[GenerateElarionWebPush]` (ADR-0076). See [Web Push](/docs/capabilities/web-push).

| Id | Severity | Meaning | Fix |
| --- | --- | --- | --- |
| ELWP001 | Error | A context is annotated with `[GenerateElarionWebPush]` but not `[GenerateDbSets]`, so the Web Push `DbSet`s and model-configuration seam are not generated. | Add `[GenerateDbSets]` to the context alongside `[GenerateElarionWebPush]`. |
