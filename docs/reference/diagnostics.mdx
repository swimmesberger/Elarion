...
## ELBLB — Blob storage (PostgreSQL)

Blob-table generation for `[GenerateElarionBlobStorage]`. See [Blob storage](/docs/capabilities/blob-storage).
...
## ELTUS — tus staging (PostgreSQL)

Tus upload-staging generation for `[GenerateElarionTusStorage]`. See
[Blob uploads](/docs/capabilities/blob-uploads).

| Id | Severity | Meaning | Fix |
| --- | --- | --- | --- |
| ELTUS001 | Error | A context is annotated with `[GenerateElarionTusStorage]` but not `[GenerateDbSets]`, so the tus staging `DbSet` and model-configuration seam are not generated. | Add `[GenerateDbSets]` to the context alongside `[GenerateElarionTusStorage]`. |
