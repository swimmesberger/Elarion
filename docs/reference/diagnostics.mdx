...
| ELACT007 | Error | `[ConsumeEvent]` is placed on an `[Actor]` class or one of its methods. Direct consumption would either bypass the mailbox or lose the consumer pipeline's inbox dedupe, and domain events must not await into actors at all (a Plane A consumer shares the caller's transaction; an actor cannot). | Write a handler-form `[ConsumeEvent]` relay consumer that calls the actor's facade — inbox dedupe then runs before the call enters the mailbox. See [Actors and events](/docs/concepts/actors#actors-and-events). |
