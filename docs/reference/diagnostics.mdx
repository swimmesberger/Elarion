...
| ELSQL011 | Error | A `[SqlRecord]` type declares a member colliding with a generated self-mapping member: `SqlMapper`, `Table`, `Select` — or, under the Npgsql COPY trigger (ADR-0068), `CopyCommandText`, `CopyTableName`, `CopyColumnList`, `WriteRowAsync`. | Rename the member; a mapped column can keep its SQL name via `[SqlColumn("…")]`. |
