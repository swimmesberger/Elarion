...
- **Triggers:** marks a plain class as an in-memory actor: its public async methods (`Task`/`Task<T>`/`ValueTask`/`ValueTask<T>`) become the message surface of a source-generated typed facade (`I{Name}`, class name minus the `Actor` suffix), and every call runs sequentially on the activation's mailbox. Keyed when the constructor takes an `IActorContext<TKey>` (one activation per key, activated on first message, passivated when idle) or `KeyType` is set; otherwise a process singleton. Registered per module via the generated `Add{Module}Actors`; requires the assembly opt-in `[assembly: GenerateActors]` (or `[UseElarion]`).
...
See [Actors — durable state](/docs/concepts/actors#durable-state-snapshots).
...
See [Actors — single-homed actors](/docs/concepts/actors#single-homed-actors-the-home-lease).
...
- **Triggers:** enables the core Elarion source-generation features at once — equivalent to applying `[GenerateModuleHandlers]`, `[GenerateModuleServices]`, `[GenerateScheduledJobs]`, `[GenerateEventConsumers]`, and `[GenerateResiliencePolicies]`, and it also subsumes the catalog triggers `[GeneratePermissionCatalog]` and `[GenerateVariantCatalog]` (each usable standalone, below). Application-owned attributes such as pipeline defaults remain explicit. Marker only — no parameters.
...
<Cards>
  <Card title="Diagnostics" href="/docs/reference/diagnostics">Compiler diagnostic ids (ELMOD/ELHTTP/ELRPC/ELMCP/ELEVT/ELKEY/ELEFC/ELAPI/ELSG/ELPIPE/ELVAL/ELFEAT/ELVAR/ELAUTH/ELPOL/ELPERM/ELRES/ELRG/ELIDN/ELIDEM/ELINBX/ELIDEMEF/ELOBX/ELSET/ELBLB/ELCACHE) referenced above.</Card>
