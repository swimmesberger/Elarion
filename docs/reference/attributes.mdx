...
- **Triggers:** applied to a partial DbContext interface; generates a `DbSet<T>` property for each `[EntityConfiguration]` entity, the matching `DbSet<T>` properties on implementing DbContext classes, and `ConfigureEntities(ModelBuilder)` on implementing DbContext classes (reflection-free, AOT-friendly direct `ApplyConfiguration<T>` calls). Class-side generation is inferred from the implementing partial `DbContext`; never place `[GenerateDbSets]` on the class.

| Parameter | Type | Default | Meaning |
| --- | --- | --- | --- |
| `scopes` | `params string[]` (ctor) → `Scopes` (`IReadOnlyList<string>`) | empty | Optional context scopes generated for this interface. Omit scopes to include every `[EntityConfiguration]`; a scoped interface includes only configurations whose scopes intersect. |
...
- **Triggers:** marks a module's published cross-module surface — the stable type other modules may depend on. The owning module keeps the implementation `internal`; other modules inject the contract. `ModuleBoundaryAnalyzer` (`ELMOD002`) reports cross-module references to a module's internal **code** — a `[Service]`, a handler, or an `[EntityConfiguration]` — but allows references to a `[ModuleContract]` type. Shared-kernel **entities** are never flagged (they are shared data, reached by every module through `IAppDbContext` by design). Marker only — no parameters.
