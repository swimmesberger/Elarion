...
- **Triggers:** marks the host assembly to have the fixed-name `ElarionBootstrapper` static generated by `AppModuleDiscoveryGenerator` (in the host's root namespace; see [ADR-0016](/docs/decisions)) — the single transport-wiring path; emits per-module and aggregate `Map`/`Add` methods for HTTP, JSON-RPC, and MCP, all module-scoped and feature-flag gated. Marker only — no parameters.
