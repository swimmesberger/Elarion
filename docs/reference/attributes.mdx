...
<Callout type="warn">
  **Runtime:** the resilience runner was extracted out of Elarion core ([ADR-0017](https://github.com/swimmesberger/Elarion/blob/main/docs/decisions/0017-dependency-light-core.md)). `[Resilient]`
  ...
  Microsoft/Polly-backed `IResiliencePipelineRunner` with `services.AddMicrosoftResilienceRuntime()`. **Breaking:**
  `AddInMemoryScheduler` no longer auto-wires the runner, so deferred scheduler retries opt in through this package too.
</Callout>
...
<Cards>
  <Card title="Diagnostics" href="/docs/reference/diagnostics">Compiler diagnostic ids (ELMOD/ELHTTP/ELRPC/ELMCP/ELEVT/ELKEY/ELEFC/ELAPI/ELSG/ELPIPE/ELFEAT/ELVAR/ELAUTH/ELPOL/ELPERM/ELRES/ELRG/ELIDN/ELIDEM/ELIDEMEF/ELCACHE) referenced above.</Card>
