...
- **Triggers:** opts the actor into Orleans-style turn-based interleaving: while one message awaits, the mailbox may start another, but turns never run in parallel (an exclusive scheduler serializes every continuation). State observed across an `await` may have been changed by an interleaved message; `ConfigureAwait(false)` inside a reentrant actor escapes the scheduler and forfeits the guarantee.
