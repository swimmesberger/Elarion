...
- **Triggers:** on a `[GenerateDbSets]` partial DbContext; the bundled generator emits `DbSet<OutboxMessage>` and `DbSet<OutboxDelivery>` and applies the transactional-outbox model configuration through the EF generator's model-config seam. The hand-written equivalent is `modelBuilder.UseElarionOutbox()`. Requires `[GenerateDbSets]` (else `ELOBX001`).
...
| `DeliveryTableName` | `string?` (init) | `null` | The per-consumer delivery table name, or `null` for `elarion_outbox_deliveries` / `ElarionOutboxDeliveries`. |
| `Schema` | `string?` (init) | `null` | The schema, or `null` for the provider's default schema. |
