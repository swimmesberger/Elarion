...
- **Triggers:** configures — or opts out of — the **inbox** on a handler-form integration-event consumer. The inbox is **on by default** for every handler whose request is an `IIntegrationEvent`: the generator attaches a `Consumer`-scoped `IdempotencyDecorator` that claims *(consumer, message id)* inside the consumer's own transaction, so a redelivered message is acknowledged instead of re-run (ADR-0022). The attribute exists to opt out (`Enabled = false` — the plain transaction decorator returns) or to tune retention; its absence means the default inbox applies. `[Inbox]` on a non-integration-event handler is inert (`ELINBX001`). Domain-event consumers never get an inbox (exactly-once by atomicity); method-form consumers have no pipeline to attach to.

| Parameter | Type | Default | Meaning |
| --- | --- | --- | --- |
| `Enabled` | `bool` (init) | `true` | Set `false` to opt out — the consumer handles duplicates itself (naturally idempotent effect, or a downstream keyed on the message id). |
| `RetentionHours` | `int` (init) | `24` | How long a completed inbox claim is retained, in hours. Must exceed the delivery tier's maximum retry window (`ELINBX002` if non-positive). |
...
<Cards>
  <Card title="Diagnostics" href="/docs/reference/diagnostics">Compiler diagnostic ids (ELMOD/ELHTTP/ELRPC/ELMCP/ELEVT/ELKEY/ELEFC/ELAPI/ELSG/ELPIPE/ELVAL/ELFEAT/ELVAR/ELAUTH/ELPOL/ELPERM/ELRES/ELRG/ELIDN/ELIDEM/ELINBX/ELIDEMEF/ELOBX/ELSET/ELBLB/ELTUS/ELCACHE) referenced above.</Card>
