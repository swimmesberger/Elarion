...
Diagnostics `ELSG014` (contract not an interface/abstract class) / `ELSG015` (empty set) / `ELSG016` (same contract declared twice) / `ELSG017` (generic implementation) / `ELSG018` (implementation also a `[Service]` under the same contract) / `ELSG019` (invalid method shape). See [Services — module services vs. contract sets](/docs/concepts/services#module-services-vs-contract-sets).
...
See [Telemetry](/docs/capabilities/telemetry) and [Connections — the low-allocation dispatch profile](/docs/capabilities/connections/low-allocation).
...
| `Scope` | `HandlerCacheScope` (init) | `Global` | `Global` evicts the globally shared entries **and** every user's `CurrentUser`-scoped entries for the tags (safe over-invalidation — the mutating caller is usually not the user whose cached read must be evicted); `CurrentUser` clears only the invoking user's entries. |
...
| `ResourceTypeName` | `string?` (init) | `null` | Explicit grants-table discriminator overriding the default (`ResourceType.FullName`). Set the same string on the matching `[ResourceFilter].ResourceTypeName` and on every stored grant; compared ordinally. |
...
- **Package:** `Elarion.Abstractions` · **Applies to:** class (generic attribute, `where TEntity : class`)
...
- **Package:** `Elarion.EntityFrameworkCore` · **Applies to:** class
- **Triggers:** on a `[GenerateDbSets]` partial DbContext; the bundled generator implements the EF model-configuration seam with a call to `ApplyElarionTenantScoping`, which attaches a named query filter to every entity implementing `ITenantScoped<TTenantId>`. Takes no parameters — which entities are tenant-scoped is a property of the entities, not of the context. Covers the **read** leg only: register `AddElarionTenantScoping()` and `AddElarionTenantScopingEntityFrameworkCore<TDbContext>()` for the tenant value and the insert-time stamp. The hand-written equivalent is `modelBuilder.ApplyElarionTenantScoping(this)` as the last statement of `OnModelCreating`. Requires `[GenerateDbSets]` (else `ELTEN001`).

See [Multi-tenancy](/docs/capabilities/multi-tenancy).
...
- **Package:** `Elarion.WebPush.EntityFrameworkCore` · **Applies to:** class
- **Triggers:** on a `[GenerateDbSets]` partial DbContext; the bundled generator emits the `DbSet<PushSubscriptionEntity>`/`DbSet<VapidKeyEntity>` and applies the Web Push model configuration through the EF generator's model-config seam — the tables backing `IPushSubscriptionStore` and `IVapidKeyStore` (ADR-0076). The hand-written equivalent is `modelBuilder.UseElarionWebPush()`. Mirrors `[GenerateElarionDeviceIdentity]`. Requires `[GenerateDbSets]` (else `ELWP001`).

| Parameter | Type | Default | Meaning |
| --- | --- | --- | --- |
| `SnakeCase` | `bool` (init) | `true` | Whether table/column names use snake_case. |
| `SubscriptionTableName` | `string?` (init) | `null` | The push subscription table name, or `null` for the default (`elarion_push_subscriptions` / `ElarionPushSubscriptions` depending on `SnakeCase`). |
| `VapidKeyTableName` | `string?` (init) | `null` | The VAPID key table name, or `null` for the default (`elarion_vapid_keys` / `ElarionVapidKeys` depending on `SnakeCase`). |
| `Schema` | `string?` (init) | `null` | The schema, or `null` for the provider's default schema. |

See [Web Push](/docs/capabilities/web-push).
