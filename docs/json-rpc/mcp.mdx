...
builder.Services.AddElarionMcp(
    RpcMethodMap.McpMetadata(),     // reflection-free tool table (MCP-surfaced methods only)
    serializerOptions,
    RpcMethodMap.RegisterMcpAll,    // builds the dedicated MCP dispatcher
...
`RpcMethodMap.McpMetadata()` and `RpcMethodMap.RegisterMcpAll` are generated alongside `RegisterAll` by the
[source generator](/docs/source-generation): a reflection-free tool table plus the registration delegate for
the dedicated MCP dispatcher. There is no assembly scanning at runtime.
...
## Module-based hosting

In a [module-based](/docs/concepts/modules) host, the generated `ModuleBootstrapper` exposes the gated MCP
peers alongside the JSON-RPC and HTTP ones:

```csharp
builder.Services.AddJsonRpc(serializerOptions, ModuleBootstrapper.RegisterRpcMethods); // /rpc (optional)

builder.Services.AddElarionMcp(
    ModuleBootstrapper.GetMcpMetadata(builder.Configuration), // gated tool table
    serializerOptions,
    ModuleBootstrapper.RegisterMcpMethods,                    // gated MCP dispatcher
    o => o.ServerName = "MyApp");
```

`GetMcpMetadata` and `RegisterMcpMethods` only include the MCP-surfaced handlers of enabled modules, so a
module disabled with `Modules:{Name}:Enabled = false` drops its tools entirely.
...
fit a flags enum. A handler can be on all three transports at once — see [HTTP endpoints](/docs/http-endpoints).
...
```csharp
builder.Services.AddElarionMcp(
    RpcMethodMap.McpMetadata(),
    serializerOptions,
    RpcMethodMap.RegisterMcpAll,
    o => {
