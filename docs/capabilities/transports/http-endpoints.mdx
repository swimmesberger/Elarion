...
`MapElarion` calls each enabled module's `MapEndpoints` hook and its generated `Map{Module}Http`
...
app.MapElarion(app.Configuration);   // module routes, gated per module
```
...
`MapElarion` maps every enabled module's routes onto the same route builder, leaving authentication
...
(and its `MapEndpoints` hook) are mapped onto. The host keeps calling `MapElarion` once; the module
owns its group:
...
third-party/public consumers; reach for [`[RpcMethod]`](/docs/capabilities/transports/json-rpc) when one team owns both ends and
