...
```xml
<ItemGroup>
  <PackageReference Include="Elarion.AspNetCore.OpenApi" Version="0.2.7" />
</ItemGroup>
```
...
- **Validation constraints, for free.** The DataAnnotations attributes on your request DTOs (see
  ...
  `[Base64String]` onto the JSON Schema keywords, verified to work reflection-off. Elarion adds small schema
  transformers for `[EmailAddress]` → `format: "email"` and `[AllowedValues]` → `enum` parity, so the OpenAPI document and the JSON-RPC schema
