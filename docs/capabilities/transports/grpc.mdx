...
```xml
<PackageReference Include="Elarion.Grpc.AspNetCore" Version="0.2.7" />
```
...
marker (`IQuery<TSelf, TResponse>`, `ICommand<TSelf, TResponse>`, or `IStreamRequest<TSelf, TItem>`) need
no explicit generic arguments: `InvokeUnaryAsync(request, context)` and
`InvokeServerStreamingAsync(request, context)` infer both.
