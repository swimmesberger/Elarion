...
```xml
<ItemGroup>
  <PackageReference Include="Elarion.AspNetCore.Mcp" Version="0.2.7" />
</ItemGroup>
```
...
adapter surfaces with the `HandlerTransports` flags (`JsonRpc`, `Mcp`, `Connection`, or `All`). The default
is `All` — JSON-RPC, MCP, and bidirectional connection adapters:
...
fit a flags enum. A handler can be on all three attribute-selected transports at once — see [HTTP endpoints](/docs/capabilities/transports/http-endpoints).
