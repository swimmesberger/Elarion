...
| `ConnectionHandlerInvoker` | `Elarion` | Connection-boundary variant of the invokers: captures one `ClientConnection` snapshot, seeds it (plus its `ClaimsPrincipal`) under exact types so adapter enrichment cannot spoof framework identity, and filters named dispatch through `HandlerTransports.Connection` |
...
Bidirectional connection adapters (the codec of an `Elarion.Connections` transport) use
`ConnectionHandlerInvoker` instead of hand-rolling the scope: it captures the connection's immutable
identity snapshot exactly once per call, so an identity promotion racing the dispatch affects the next
message rather than the scope underneath the current one:

```csharp
var result = await ConnectionHandlerInvoker.InvokeAsync<StartCommand, StartAck>(
    rootProvider, connectionSink, decoded, ct: ct);
...
var named = await ConnectionHandlerInvoker.InvokeNamedAsync(
    rootProvider, connectionSink, dispatcher, name, decodedRequest, ct: ct);
```
