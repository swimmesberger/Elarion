...
[gRPC adapter](/docs/capabilities/transports/grpc): it owns
...
| `HandlerDispatcher` | `Elarion.Abstractions` (namespace `Elarion.Abstractions.Dispatch`) | Named request/reply bus with no wire format |
| `DispatchScopeContext`, `CreateDispatchScope`, `SeedScope` | `Elarion.Abstractions` (namespace `Elarion.Abstractions.Dispatch`) | Per-call scope-seeding rail |
...
| `ConnectionHandlerInvoker` | `Elarion` | Connection-boundary variant of the invokers, bound once per connection: each call captures one `ClientConnection` snapshot, seeds it (plus its `ClaimsPrincipal`) under exact types so adapter enrichment cannot spoof framework identity, and filters named dispatch through `HandlerTransports.Connection` |
...
// Requests declaring their response type via a self-typed marker (ICommand<TSelf, TResponse> /
// IQuery<TSelf, TResponse>) need no explicit generic arguments:
var inferred = await HandlerInvoker.InvokeAsync(rootProvider, command, context, ct);
```

Bidirectional connection adapters (the codec of an `Elarion.Connections` transport) bind a
`ConnectionHandlerInvoker` once per connection instead of hand-rolling the scope: each call captures the
connection's immutable identity snapshot exactly once, so an identity promotion racing the dispatch
affects the next message rather than the scope underneath the current one:

```csharp
var invoker = new ConnectionHandlerInvoker(rootProvider, connectionSink);

var result = await invoker.InvokeAsync(new StartCommand(...), ct);   // inferred via self-typed marker
var explicitResult = await invoker.InvokeAsync<StartCommand, StartAck>(decoded, ct);

// Decoded named traffic is filtered to routes exposed with HandlerTransports.Connection; unknown and
// other-transport names return the same generic NotFound so exposure is never leaked.
...
dispatcher.MapDelegate<PingRequest, PongResponse>("ping", (request, services, ct) => HandlePing(request, ct));
...
the typed seam. Startup errors are normal `AppError`s; keep the accepted invocation alive while writing items:
...
direct `MapGet` lambda in the host compilation, where ASP.NET Core owns binding. The lazy result uses
...
generic equivalent: a concrete per-endpoint registration keeps the request/response types visible to the
trimmer and to OpenAPI, and its binding code concrete and AOT-safe (ADR-0071). A reusable feature therefore
ships a concrete `MapElarionX(route)` extension.
