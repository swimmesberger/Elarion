...
`MapElarionHandlerStream` is the shipped ASP.NET SSE adapter; it uses the native
`TypedResults.ServerSentEvents` framing. JSON-RPC and MCP are deliberately single-response;
...
its JSON-RPC/MCP protocol adapters, and `Elarion.AspNetCore` only when hosting in ASP.NET Core. Core must
never reference the new adapter.
