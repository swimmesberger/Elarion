...
dispatcher.MapDelegate<Ping.Request, Ping.Response>("diagnostics.ping", (req, services, ct) => /* ... */);
```
...
dispatcher translates `AppError.Kind` into a JSON-RPC error code through the default
`IAppErrorTranslator<RpcError>` (`AppErrorMapper`). A host that needs different codes registers its own
`IAppErrorTranslator<RpcError>`, which the JSON-RPC and MCP adapters resolve instead of the default.
