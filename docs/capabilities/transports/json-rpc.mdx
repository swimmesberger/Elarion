...
3. The host configures a `JsonRpcDispatcher` with the same `JsonSerializerOptions` used at runtime; it
   adapts the shared `HandlerDispatcher` registry to the JSON-RPC wire format.
