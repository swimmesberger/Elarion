...
// rail JSON-RPC and MCP use. Authorization, validation, [Idempotent], transaction, and audit all run
// per message, evaluated against the connection's captured identity snapshot (seeded as ICurrentUser
// by the standard initializer) — a promotion racing this dispatch affects the next message, never the
// scope underneath this one, and adapter metadata can never overwrite the framework identity entries.
var result = await ConnectionHandlerInvoker.InvokeAsync<StartCommand, StartAck>(
    services, connection, decoded, ct: ct);
...
var named = await ConnectionHandlerInvoker.InvokeNamedAsync(
    services, connection, dispatcher, name, decodedRequest, ct: ct);
```
