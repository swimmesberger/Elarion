...
`Elarion.Connections.Testing` ships the pieces every gateway test suite and device simulator otherwise
hand-rolls. `TestClientConnection` is an in-memory sink double — register it with the real registry and
...
completions (with `Reset()` for reconnect scenarios). `TcpTestClient` is the framed simulator client:
...
handshakes. Codec-level tests stay on real adapters — loopback TCP or Kestrel + `ClientWebSocket` — the
double covers the kernel tier.
