...
messages, unchanged. The pipeline is benchmarked at **raw-socket parity with zero allocations per
message** on the binary delivery path (`DelimitedTextTcpFramer(kind: TcpMessageKind.Binary)` delivers
the raw slice to `OnBinaryAsync` — no per-message string), and an armed-but-quiet idle window costs
nothing on the hot path; text delivery prices in one string decode per message.
