...
not feature-owned: the recommended home is a shared `Persistence` layer (a sibling of your entities, under
no `[AppModule]`), while entities themselves stay plain classes. (See
[Solution structure](/docs/concepts/solution-structure) and
[ADR-0007](https://github.com/swimmesberger/Elarion/blob/main/docs/decisions/0007-data-is-platform-module-as-plugin.md)
for the rationale; the generator discovers configurations wherever they live, so placement is a
convention.) Because the manifest is emitted per assembly, the project that declares the configurations
and the project that holds the concrete `DbContext` (commonly separate assemblies) each need this
