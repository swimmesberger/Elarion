...
abstraction. Configurations describe the one shared model every feature draws on, so they are part of the
shared **data layer**, not feature-owned: they live in a shared `Persistence` layer alongside the concrete
`DbContext` (the database is application logic — see [Philosophy](/docs/concepts/philosophy) and
