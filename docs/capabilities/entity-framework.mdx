...
business data by id; that is what `CreatedAt` is for. The **ELID001** analyzer (Warning, in the `Elarion`
package) flags every `Guid.NewGuid()` and ships a code fix that rewrites it to `Guid.CreateVersion7()`; the
rare id that must be *unpredictable* is the one legitimate v4 site — suppress the diagnostic there with a
justification. See [Diagnostics](/docs/reference/diagnostics#elid--identifier-conventions).
...
lives in [ADR-0038](../decisions/0038-client-assigned-entity-identity.md).
