...
<Callout type="info">
  ...
  `UseElarionBlobStorage()`, or `[GenerateElarionBlobStorage]` on a `[GenerateDbSets]` context), so they
  join your context's model without opting into entity generation — see
  [Blob storage](/docs/capabilities/blob-storage).
</Callout>
...
reflection-free (trim/AOT-safe) and never moves when a host retunes its wire JSON.
