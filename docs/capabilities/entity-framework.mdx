...
abstraction. Configurations are a persistence concern, so they live with your modules in the
application layer (beside the handlers), **not** in the domain — entities themselves stay plain
classes. Because the manifest is emitted per assembly, the project that declares the configurations
and the project that holds the concrete `DbContext` (commonly separate assemblies) each need this
reference: the former so its generator emits the per-assembly configuration manifest, the latter so
the generator reads that manifest to fill its context with `DbSet`s:
