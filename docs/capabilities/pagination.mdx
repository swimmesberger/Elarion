...
The effective size matters because the server [clamps `Size`](#keyset-pagination) to the handler's
`maxSize` (default `100`) and substitutes `20` for anything below `1` — and `Page<T>` does not echo the
size back. Divide by the size the server would actually have used, or the last page number is wrong
whenever a client over-asks:
