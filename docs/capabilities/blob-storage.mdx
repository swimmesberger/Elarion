...
builder.Services.AddElarionPostgreSqlBlobStore<AppDbContext>(connectionString);
```

The connection string (or an `NpgsqlDataSource`, if you prefer the parameterless overload and register
one yourself — a host-registered data source always wins) backs the store's **streaming reads**: each
`OpenReadAsync` draws a dedicated connection whose lifetime is owned by the returned `BlobDownload`,
which the scoped `DbContext` connection cannot provide. It must target the same database as the blob
context.
...
`bytea` column without buffering. Reads stream too: `OpenReadAsync` opens a dedicated connection from
the registered `NpgsqlDataSource` and reads through `CommandBehavior.SequentialAccess` +
