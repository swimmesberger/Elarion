...
| `Placement` | `JobPlacement.Cluster` (default — one node per occurrence when a coordinator is registered) or `EveryNode`. See [Multi-node coordination](/docs/capabilities/scheduling/multi-node). |

See [Overlap & misfire](/docs/capabilities/scheduling/overlap-and-misfire) for `Group`, `Overlap`,
`MisfirePolicy`, and `MaxConcurrentRuns` in detail.
