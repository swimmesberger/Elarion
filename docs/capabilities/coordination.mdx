...
| Deliver one integration message at a time | [Outbox lease](/docs/capabilities/events/backends) | Message |
...
`"actors"` role for `[Actor(SingleHomed = true)]` actors. Calls on other instances fail rather than
forwarding through the actor runtime; host-owned HTTP prefixes can use
`UseElarionRoleHolderProxy("actors", ...)` to reach the home. See [Actors](/docs/concepts/actors) for
