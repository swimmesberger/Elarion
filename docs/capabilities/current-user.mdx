...
Elarion bridges that gap explicitly (no `IHttpContextAccessor`, no `AsyncLocal`): each transport captures
the authenticated principal at its boundary into a `DispatchScopeContext`, and `AddElarionCurrentUser()`
registers an `IDispatchScopeInitializer` that seeds the per-call snapshot from it. Authorization rides the
same path, since the authorizer reads `ICurrentUser`.

`IDispatchScopeInitializer` is the public extension point for this. To carry any other per-call scoped
state (tenant, correlation id, …) into JSON-RPC/MCP handler scopes, register your own:
...
internal sealed class TenantScopeInitializer : IDispatchScopeInitializer {
    public void Initialize(IServiceProvider callScope, DispatchScopeContext context) {
        if (context.TryGet<ClaimsPrincipal>(out var user) && user is not null) {
            callScope.GetRequiredService<TenantContext>().TenantId =
