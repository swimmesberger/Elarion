...
| One request returns a lazy export, token output, or tail query | [`IStreamHandler`](/docs/capabilities/transports/server-streaming) — a cold request response |
| Every browser-visible item needs ordering, completion, and reconnect resume | An actor-owned [ordered stream](/docs/capabilities/events/streams) |
...
[`IStreamHandler`](/docs/capabilities/transports/server-streaming); a browser-visible sequence whose elements,
completion, and resume behavior are data is an actor-owned [ordered stream](/docs/capabilities/events/streams).
...
internal sealed class CustomerScopeAuthorizer(AppDbContext db, ICurrentUser user)
    ...
    public async ValueTask<bool> AuthorizeAsync(ClientEventSubscription subscription, CancellationToken ct) =>
        ...
        && await db.CustomerMembers.AnyAsync(m => m.CustomerId == customerId && m.UserId == user.UserId, ct);
}
```
...
- **Every element matters, browser**: publish the *hint* and let the re-query be the ordered, complete
  ...
  [ordered-streams tier](/docs/capabilities/events/streams) (ADR-0052): a real stream contract with
  order, completion, and `Last-Event-ID` resume. It is a separate contract — never bolted onto hints.
</Callout>
...
await clientEvents.PublishAsync(
    ...
    ClientEventScope.User(user.UserId), ct);
```
