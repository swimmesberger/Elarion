...
```csharp
// The command: create-without-a-natural-key is the classic double-submit case — a client retry
...
The rule of thumb that falls out: **write the consumer and ship it — the default is safe. Declare
`[AllowDuplicates]` only when you can name the reason re-running is harmless; declare `[Idempotent]` on the
command when a client retry could create something twice** (see
[when does a command need it](/docs/concepts/idempotency#when-does-a-command-need-it)).
