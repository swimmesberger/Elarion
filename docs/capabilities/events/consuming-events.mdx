...
- **Retention must outlive retries.** Inbox rows expire like idempotency keys (default 24 h, tune with
  `[Inbox(RetentionHours = …)]`). Keep retention above the outbox's maximum retry window (≈43 minutes at
  the defaults), or a still-retrying message could re-run after its claim was purged.
...
**Opting out.** `[Inbox(Enabled = false)]` removes the inbox (the plain transaction decorator returns) for
consumers that need no dedup rows — the effect is naturally idempotent, or its only effect is a call to a
keyed downstream:

```csharp
[ConsumeEvent]
[Inbox(Enabled = false)] // "mark paid only if not already paid" converges by itself
