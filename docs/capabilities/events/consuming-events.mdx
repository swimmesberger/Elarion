...
reported (`ELEVT005`/`ELEVT001`); an invalid signature is `ELEVT002`.
...
multiple instances — or a fan-out sibling that failed: consumers sharing a target group (in the common
unbound case, every consumer of the event) share one retry boundary, so a retry re-runs the siblings that
already completed. There is no exactly-once
*delivery*; exactly-once *effect* comes from deduplication on the consumer side. Every redelivery carries
the **same** `IEventContext.MessageId` (the durable message identity — for the outbox, the published
message's id, shared by every target group of one publish), so
...
**grain**. It is one row per *event* (per target group), finalized by the delivery worker per **group**, on
the worker's own connection, *after* your consumer's transaction has already committed — and only once
**every** fan-out consumer in that group has run. Your effect is a fact per *(message, consumer)*, and only a write inside your
...
- **Durability follows the idempotency store.** The delivery tiers register the in-memory store
  ...
  [`AddElarionIdempotencyEntityFrameworkCore`](/docs/concepts/idempotency) so inbox claims survive
...
The classic CRUD-with-side-effects shape — create a row, generate a PDF, send an email, keep another
module's statistics current — maps onto the pieces like this:
...
[ConsumeEvent]
[AllowDuplicates]
public sealed class RecalculateStatistics(StatsDbContext db) : IHandler<InvoiceCreated> { … }
```

The rule of thumb that falls out: **defaults need no justification; deviations must name their reason.**
...
increments (see [when does a command need it](/docs/concepts/idempotency#when-does-a-command-need-it)).
...
builder.Services.AddSingleton(new EventSubscriptionDescriptor {
    ConsumerId  = "MyApp.InvoiceNotifications.OnInvoiceCreated(MyApp.InvoiceCreated)", // stable; the outbox requires it
