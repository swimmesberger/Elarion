...
two-plane model itself, see the [events overview](/docs/capabilities/events).)
...
- **`Result<Unit>` (or the `IHandler<TEvent>` sugar) → a fan-out subscriber.** Every matching consumer
  runs. A failed `Result` has no return channel to the publisher, so it surfaces as an
...
- **`Result<T>` with `T ≠ Unit` → the single `RequestAsync` responder** for the event, returning its
  typed result through the same pipeline. Domain plane only.
...
- `Result<TResponse>` / `Task<Result<TResponse>>` / `ValueTask<Result<TResponse>>` → the **single
  responder** for `IDomainEventBus.RequestAsync` (exactly one allowed; domain plane only).
