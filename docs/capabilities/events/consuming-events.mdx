...
Here too the method is a **fan-out subscriber** — it returns `void` / `Task` / `ValueTask` (every matching
consumer runs). The event bus is pub/sub-only, so a method that returns a `Result<TResponse>` is rejected
