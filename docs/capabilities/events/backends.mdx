...
transaction (via the transaction decorator on the delivery scope); the worker then finalizes the message —
a separate lease-guarded write on the worker's connection, per message, once **all** fan-out consumers
have run. Two consequences are inherent: a crash between a consumer's commit and the finalize redelivers
the message, and one failed consumer retries the *whole* message, re-running siblings that already
succeeded. That is what at-least-once means in practice — write consumers to absorb it (see
