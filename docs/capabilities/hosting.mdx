...
`AddElarionJsonRpc(serializerOptions, ModuleBootstrapper.RegisterRpcMethods)` is the **config-aware** overload: it
resolves `IConfiguration` when the dispatcher singleton is built and registers only the enabled modules'
JSON-RPC-surfaced `[RpcMethod]` handlers. `MapElarionEndpoints` does the same gating for module `MapEndpoints`
