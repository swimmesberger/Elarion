...
| `Elarion.Blobs.Tus` | Host | The tus 1.0 resumable transport (`MapElarionTus`, `AddElarionTus`) — a pure protocol adapter over the staging seam. |
...
session retention) lives in `TusOptions` and reaches the store as data, so staging backends are
...
session row is retained for `TusOptions.CompletedSessionRetention` (default 1 hour) so a client `HEAD`
