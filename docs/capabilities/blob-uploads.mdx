...
| `Elarion.Blobs.PostgreSql` | Infrastructure / host | The lifecycle on the PostgreSQL store, `state`/`expires_at` columns with a partial index, the `BlobGarbageCollector` sweeper, and `AddElarionPostgreSqlBlobLifecycle`. |
...
services.AddElarionTusPostgreSql<AppDbContext>(connectionString);
...
(idempotently, via `AddElarionPostgreSqlBlobLifecycle`) — a completed tus upload produces a *pending* blob, so
without that collector an abandoned upload would leak its blob forever. Map the blob tables with
`UseElarionBlobStorage()` alongside the staging table. Both wiring methods take optional `tableName`
(plus `contentTableName` on the blob side) and `schema` parameters and a `snakeCase` toggle (default
`true`; `false` switches to PascalCase names — `TusUploads`, `StoredBlobs`/`BlobContents`); the stores'
raw Npgsql SQL (content streaming and the tus conditional append) is built from the EF model, so the
overrides apply there too. On a `[GenerateDbSets]` context, `[GenerateElarionBlobStorage]` and
`[GenerateElarionTusStorage]` (each with optional `SnakeCase`/`TableName`/`Schema` — the blob attribute
also takes `ContentTableName`) replace the hand-written calls: the bundled generators emit the `DbSet`s
and apply the same configuration through the EF generator's model-config seam (`ELBLB001`/`ELTUS001` if
`[GenerateDbSets]` is missing). A completed session row is retained for
