...
3. The handler streams from `IBlobStore.OpenReadAsync(...)` and processes; it commits the blob only if it
   should be kept. An abandoned or fully-consumed import is swept by the garbage collector.
