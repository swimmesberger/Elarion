...
overload carries the individual messages for the transport to surface. The
`where TResponse : IResultFailureFactory<TResponse>` constraint lets the decorator build the failure
result via the static `TResponse.Failure(error)` — no reflection — and also scopes it at compile time
to `Result`-returning handlers (see
[decorator pipelines](/docs/concepts/decorator-pipelines#turning-an-apperror-into-tresponse)).
