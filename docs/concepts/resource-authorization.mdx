...
The generator completes the class as an `IQueryAuthorizer<Contact>` with a static `Specification`. Compose it
into the query **before** paging:

```csharp
var page = await db.Contacts
    .WhereAuthorized(ContactAccess.Specification, currentUser)   // owner OR shared-with-user OR shared-with-role
