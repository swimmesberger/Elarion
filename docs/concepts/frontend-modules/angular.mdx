...
// snapshot can change at runtime? provideContributions(createContributionRegistryStore(manifests, capabilityStore))
// and capabilityStore.set(...) on login (a Signal<ContributionRegistry> you drive yourself also works).
```
...
The slot site then reads like the React bindings' `<ExtensionSlot>`, with the contribution as the template's
implicit value:
