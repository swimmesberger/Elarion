...
This registers `HybridCache` and the default `IHandlerCache` (`HybridHandlerCache`). To use a
different backing store, configure `HybridCache` with a distributed cache (for example Redis) as you
normally would, or register your own `IHandlerCache` implementation.
