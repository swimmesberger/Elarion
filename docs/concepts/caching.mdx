...
| Scope | Behavior |
| --- | --- |
| `CurrentUser` (default) | Entries and tags are isolated by the authenticated user. Requires an [`ICurrentUser`](/docs/concepts/current-user) with a user id; the id is hashed before it enters the physical cache key. |
...
[trace-visible](/docs/telemetry): get/create spans report the precise outcome —
