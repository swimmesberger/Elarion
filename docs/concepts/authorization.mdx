...
request shape (e.g. only writes) without touching the handler.
...
**explicit** `Require*` still gets the decorator — so a rule reachable from a consumer must tolerate an
unauthenticated principal.

`[AllowAnonymous]` handlers opt out of authorization entirely, global rules included — that attribute means "this
operation is outside authorization", and a rule must not resurrect a gate the handler declined. A check that must
also cover anonymous traffic belongs in the handler or in host middleware.
