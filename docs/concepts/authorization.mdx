...
[RequirePermission("tenants", Verbs.Write)]
...
| `[RequirePermission("x", Verbs.Read)]` | Sugar for `[RequireClaim(PermissionClaimType, "x.read")]` — the composed `{resource}.{verb}` claim over the configured permission claim type (default `"permission"`). The common case. |
...
The default forbidden message is a generic `"Access denied."` — it deliberately does not name the unmet
permission/role/policy, so a forbidden caller cannot probe the permission vocabulary (the same opacity
the feature-gate 404 applies). The default authorizer logs the unmet requirement at debug level for
operators; a host that wants the detail on the wire sets `AuthorizationOptions.ForbiddenMessageFormat`
to a format containing `{0}` (e.g. `"Missing required permission: {0}"`).
...
<Callout type="info">
  Deny-by-default does **not** implicitly attach to **event-consumer handlers** (a handler whose request
  implements `IDomainEvent`/`IIntegrationEvent`). Consumers are dispatched on a delivery scope with no
  authenticated user, so an implicit authenticated-by-default gate would fail every consumer. An
  **explicit** `Require*` on a consumer is a deliberate app choice and still attaches; only the implicit
  default is skipped.
</Callout>

Because `Require*`/`[AllowAnonymous]` are `Inherited = true`, a requirement declared on a **base** handler
class also attaches the decorator to a derived handler (mirroring the runtime `HandlerMetadata`
`GetCustomAttributes(inherit: true)` path), and an `[AllowAnonymous]` on the derived handler wins over an
inherited base requirement.
...
### Global rules — one condition across every handler

A named policy answers "does *this* handler's declared requirement hold". Some conditions are not per-handler at
all: a suspended tenant, a lapsed subscription, a read-only maintenance window. Register an
`IGlobalAuthorizationRule` and it runs for **every** authorized invocation — after the authenticated gate, before
the declared permission/role/claim/policy/resource checks:
...
public sealed class ActiveSubscriptionRule(ITenantContext tenant, ITenantDirectory directory) : IGlobalAuthorizationRule {
    public async ValueTask<AppError?> EvaluateAsync(AuthorizationContext context, CancellationToken ct) {
        ...
        return suspended ? AppError.Forbidden("This workspace is suspended.") : null;   // null => pass
    }
}
...
Rules are additive and run in **registration order**; the first non-null `AppError` wins and reaches the caller
**unchanged**, so the rule — not the framework — chooses the outcome kind. A rule that must not disclose a
resource returns `AppError.NotFound(...)`; one that states the denial plainly returns `AppError.Forbidden(...)`.
The `AuthorizationContext` carries the current user and the handler request, so a rule can scope itself to a
request shape (e.g. only writes) without touching the handler. On the **client-event subscription** path there is
no request, so `context.Resource` is `null` — a rule that branches on the request shape will fall through to its
denial there, which is fail-closed but may not be what you meant. Handle `null` explicitly if a rule should treat
subscribes differently from commands.

<Callout type="warn">
A rule only runs **where the authorization decorator is attached**, and the generator attaches it at compile time
only for handlers carrying a `Require*` attribute or in scope of `[ElarionAuthorizationDefaults]`. For broad
coverage, pair the rule with the assembly-level default:

```csharp
[assembly: ElarionAuthorizationDefaults] // RequireAuthenticated = true
```

That reaches every handler **except event-consumer handlers**, which the deny-by-default section above exempts
from *implicit* attachment (they run on a delivery scope with no authenticated user). A consumer carrying an
**explicit** `Require*` still gets the decorator — and is then rejected as unauthenticated on that scope, before
any rule is consulted.

Rules always run **behind the authentication gate**: a handler that declares any requirement rejects an
unauthenticated caller with `Unauthorized` before rules are reached, so a rule never has to defend against a
missing principal. `[AllowAnonymous]` handlers opt out of authorization entirely, global rules included — that
attribute means "this operation is outside authorization", and a rule must not resurrect a gate the handler
declined. A check that must also cover anonymous traffic belongs in the handler or in host middleware.

Client-event **subscriptions** are covered: any topic declaring a requirement is decided by the same `IAuthorizer`,
so a rule that denies a request also denies the corresponding subscribe.
</Callout>

### Replacing or decorating the authorizer

Every registration in `AddElarionAuthorization()` uses `TryAdd`, and the concrete `ClaimsAuthorizer` is registered
as its own service with the `IAuthorizer` registration resolving it. That pins two seams:

- **Replace** — register your own `IAuthorizer` **before** calling `AddElarionAuthorization()`; the framework then
  adds nothing for that service.
- **Decorate** — do the same, and inject the concrete `ClaimsAuthorizer` as your inner authorizer. Both resolve to
  the same scoped instance, so there is no second authorizer and no `RemoveAll` gymnastics:
...
builder.Services.AddScoped<IAuthorizer, AuditingAuthorizer>();   // registered first — TryAdd defers to it
builder.Services.AddElarionAuthorization();                      // still registers ClaimsAuthorizer itself
```

Reach for a global rule when the condition is a *decision*; reach for decoration when you need to observe or
reshape decisions the shipped authorizer already makes.

<Callout type="warn">
Only `ClaimsAuthorizer` evaluates `IGlobalAuthorizationRule`. **Wholesale replacement** of `IAuthorizer` makes
every registered rule inert unless the replacement consults them itself — decoration keeps them running, because
the inner `ClaimsAuthorizer` still does the work.
</Callout>
...
the same way services and event consumers are — each module contributes through its generated `ConfigureDefaultServices`,
...
- [Resource authorization](/docs/concepts/resource-authorization) — per-resource (read/write) checks and efficient database-level filtering.
