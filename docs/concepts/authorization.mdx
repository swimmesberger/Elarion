...
A handler *declares* the permission it needs inline (`[RequirePermission("properties.write")]`) — no central
registry to edit. But the **set of all permission strings** is still needed: to **seed** permission claims onto
roles at startup, and to drive role→permission policy. Hand-maintaining that `All`/`ReadOnly` list is the one
file that grows per feature — the "edit-for-every-module" smell the generators otherwise eliminate.

The generator removes it. It discovers every `[RequirePermission]`/`[RequireRole]` — resolving the constant when
you write `[RequirePermission(Permissions.PropertiesWrite)]` — and emits two surfaces.
...
from static role policy instead of a hand-kept list:

```csharp
// Roles.cs — no All/ReadOnly list to maintain
[Admin]   = ElarionPermissions.All,
[Manager] = [.. ElarionPermissions.All.Where(p => p != ElarionPermissions.Users.Manage)],
[Viewer]  = ElarionPermissions.ByKind[PermissionKind.Read],
```

- `All` / `Roles` — the deduplicated, ordinally sorted universe.
- `ByModule["Billing"]` — permissions grouped by owning `[AppModule]`.
- `ByKind[PermissionKind.Read]` — grouped by the optional classification on
  `[RequirePermission("x", PermissionKind.Read)]`, so a Viewer grant doesn't depend on a `.read` string-suffix
  convention.
- typed accessors — `ElarionPermissions.Users.Manage` (the first path segment becomes a nested class), so policy
  references stay type-safe and survive the permission consts moving into modules.
...
public sealed class IdentitySeeder(IPermissionCatalog catalog, RoleManager<AppRole> roles) {
    public async Task SeedAdminAsync(AppRole admin) {
        foreach (var permission in catalog.Permissions)          // every [RequirePermission] across all modules
            await roles.AddClaimAsync(admin, new Claim("permission", permission));
    }
}
```

It exposes the same `Permissions`/`Roles`/`ByKind`, plus a per-module `Modules` breakdown, populated the same way
services and validators are — each module contributes through its generated `ConfigureDefaultServices`, **gated by
module enablement**, so a disabled module's permissions never appear and a referenced module library's are
included automatically. `AddElarionAuthorization` registers it.
