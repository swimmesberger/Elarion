...
A named policy is a transport-neutral `IAuthorizationPolicy` resolved from DI by name:
...
Register it (or an inline delegate) and reference it with `[RequirePolicy("AtLeast21")]`:

```csharp
services.AddElarionAuthorization();
services.AddElarionAuthorizationPolicy<AtLeast21Policy>();
// or: services.AddElarionAuthorizationPolicy("AtLeast21", (ctx, ct) => ...);
```

> **Reusing ASP.NET policies.** If you already have ASP.NET `AddAuthorization` policies, the optional
> `AddAspNetAuthorizationPolicyBridge("Name")` (in `Elarion.AspNetCore`) exposes one as an
> `IAuthorizationPolicy`. Caveat: Elarion passes the *request* as the resource, so a policy whose handler
> casts `context.Resource` to `HttpContext` only works under HTTP. The native `IAuthorizationPolicy` is
