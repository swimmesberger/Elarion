...
only for handlers carrying a `Require*` attribute or in scope of `[ElarionAuthorizationDefaults]`. To have a
global rule cover *literally every* handler, pair it with the assembly-level default:
