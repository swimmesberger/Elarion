...
| `SchedulerRegistrationGenerator` | `[assembly: UseElarion]` or `[assembly: GenerateScheduledJobs]` | Per-module `Add{Module}ScheduledJobs()` registering descriptors and job types. |
...
| `ActorRegistrationGenerator` | `[assembly: UseElarion]` or `[assembly: GenerateActors]` | Typed actor facades and each module's `Add{Module}Actors()` registration hook. See [Actors](/docs/concepts/actors). |
| `AuthorizationPolicyRegistrationGenerator` | `[assembly: UseElarion]` or `[assembly: GenerateModuleAuthorizationPolicies]` | Per-module registration for named `[AuthorizationPolicy]` implementations. See [Authorization](/docs/concepts/authorization). |
| `ClientEventRegistrationGenerator` | `[assembly: UseElarion]` or `[assembly: GenerateClientEventTopics]` | Per-module topic catalog/registration from `IClientEvent` contracts and their subscribe-time requirements. See [Client events](/docs/capabilities/events/client-events). |
...
| `VariantCatalogGenerator` | `[assembly: UseElarion]` or `[assembly: GenerateVariantCatalog]` | The typed `ElarionVariants` catalog, grouped by module and switch. See [Feature flags](/docs/concepts/feature-flags). |
| `ElarionManifestGenerator` | Any project containing `[AppModule]`, `[ModuleEndpoints]`, `[HttpEndpoint]`, `[Handler]`, `[ResourceFilter]`, `[RequirePermission]`, `[RequireRole]`, `[FeatureVariant]`, or `[ConfigurationVariant]` | Internal assembly metadata manifests consumed by host-side generators for referenced modules and transport handlers. |
