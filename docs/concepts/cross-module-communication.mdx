...
[ADR-0002](https://github.com/swimmesberger/Elarion/blob/main/docs/decisions/0002-cross-module-communication.md)
for the rationale.
...
`[EntityConfiguration]` is platform too and lives in the shared `Persistence` layer (under no
...
<Callout type="info">
  ...
  [Google Rich Error Model](https://grpc.io/docs/guides/error/) (`google.rpc.Status`). See
  [ADR-0005](https://github.com/swimmesberger/Elarion/blob/main/docs/decisions/0005-cross-module-error-channel.md)
  for the `.proto` shapes and the `ErrorKind` → `google.rpc.Code` table. **Prefer `Result`; only drop it
  when a concrete need calls for it.**
</Callout>
