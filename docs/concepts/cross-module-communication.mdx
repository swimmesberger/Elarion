...
Injecting Module A's internal `[Service]`, a handler, an `[EntityConfiguration]`, or a configured entity
(the entity behind an `[EntityConfiguration]`, since entities carry no marker) from Module B instead of a
