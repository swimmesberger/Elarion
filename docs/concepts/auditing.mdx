...
| **The resource** | *which thing was the action about?* (the indexed "browse by resource" view) | `[Auditable(Resource = "property")]` (type only) and/or `IAuditScope.SetResource("property", id)` (type + id) |
...
| --- | --- | --- |
| `ResourceType` | `[Auditable(Resource = "property")]` **or** `SetResource(type, …)` | `"property"` |
| `ResourceId` | **only** `SetResource(type, id)` | `"42"` |

- `[Auditable(Resource = "property")]` **alone** → *"someone updated **a** property"* — a resource *type* label
...
- `audit.SetResource("property", "42")` → *"…property **#42**"* — type **and** id, so it appears on that one
...
inject `IAuditScope`, call `SetResource` with the id, and mark the entity `[Audited]`:

```csharp
[Auditable]   // no Resource — SetResource below supplies the type (and the id)
public sealed class UpdatePropertyHandler(AppDbContext db, IAuditScope audit)
    ...
    public async ValueTask<Result<PropertyResponse>> HandleAsync(UpdatePropertyCommand request, CancellationToken ct) {
        ...
        audit.SetResource("property", property.Id.ToString());   // ResourceType + ResourceId (optionally a parent)
        return Result<PropertyResponse>.Success(new() { Id = property.Id });
    }
}
```
...
- **`[Auditable]` + `SetResource("x", id)`** — you want *"everything done to **this** x"* on x's own history
  screen. Add a parent — `SetResource("equipment", id, parentType: "property", parentId: propertyId)` — to also
