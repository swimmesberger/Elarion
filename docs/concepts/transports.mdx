...
- **Named bus (JSON-RPC + MCP)** — a generic seam on the dispatcher, chained into the host's `RegisterHandlers`:
  ...
  ```csharp
  dispatcher.MapHandler<MyRequest, MyResponse>("my.operation", HandlerTransports.JsonRpc | HandlerTransports.Mcp);
  ```

  It is name-routed (you pick the transports, no route); serialization goes through the configured
  `JsonTypeInfo`. This is the same `MapHandler` the generated bootstrapper calls per `[Handler]`.
...
`MapHandler`, and a concrete `MapElarionX(route)` for REST; the host opts in and picks surfaces.
