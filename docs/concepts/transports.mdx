...
not reference any transport package — so you reference `Elarion.JsonRpc` only if you use the name-based
dispatcher, and `Elarion.AspNetCore` only if you are co-hosted in ASP.NET. The scope rail and `HandlerInvoker`
need no transport package (rail in `Elarion.Abstractions`, `HandlerInvoker` in `Elarion`).
