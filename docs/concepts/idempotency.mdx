...
Opt out with `[Inbox(Enabled = false)]` (naturally idempotent effect, or the only effect is a call to a keyed
downstream); tune retention with `[Inbox(RetentionHours = …)]` — keep it above the outbox's maximum retry
window. Rows share the idempotency table and purge loop under a `"consumer"` scope discriminator. See
...
- `ELINBX001` (warning) — `[Inbox]` on a handler whose request is not an `IIntegrationEvent` (no effect).
- `ELINBX002` (error) — non-positive `RetentionHours` on `[Inbox]`.
