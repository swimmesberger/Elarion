...
```csharp title="PacketBindingRegistrations.cs"
...
- **`[Service]`** — feature-gated module services, *pushed* by the gated module bootstrapper. The
  registration lives and dies with the module.
- **Contract sets** — unconditional infrastructure seams, *pulled* by the host from its composition
  root, exactly once. Nothing gates the composition; the registry consuming
  `IEnumerable<TContract>` validates the set at startup and is free to fail loudly on an invalid
  one. An empty set is a warning at compile time ([`ELSG015`](/docs/reference/diagnostics)) —
  almost always a typo'd contract or refactor casualty.
...
[`ELSG018`](/docs/reference/diagnostics). See
[`[GenerateContractSetRegistration]`](/docs/reference/attributes#generatecontractsetregistration)
