...
<Callout type="info">
  ...
  See [ADR-0015](https://github.com/swimmesberger/Elarion/blob/main/docs/decisions/0015-ef-core-transaction-participation.md).
</Callout>
...
public sealed class ArchiveInvoice(
    ...
    public async ValueTask<Result<Unit>> HandleAsync(Command command, CancellationToken ct) {
        ...
        await settings.SetAsync($"invoice:{command.Id}:archived", "true", cancellationToken: ct);
...
domain-event handlers (they run inline inside the publisher's transaction), `[Idempotent]`
handlers, and integration-event consumers under the default-on inbox (both own the boundary
themselves, so a handler is never wrapped in two nested transactions — only an `[AllowDuplicates]`
consumer gets the plain transaction).
...
(see [ADR-0001](https://github.com/swimmesberger/Elarion/blob/main/docs/decisions/0001-event-transaction-phase.md)):
...
<Callout type="info">
  ...
  The durable [outbox](/docs/capabilities/events/backends) tier commit-gates
