...
Prefer slot sugar over an inline `@for`? A `*extensionSlot` **structural directive** is a few lines you
own on top of `injectContributions` (the package ships no directive — that would need the Angular
compiler); the [package README](https://www.npmjs.com/package/@swimmesberger/elarion-contributions) has
the ready-to-copy version.
