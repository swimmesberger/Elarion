...
- Related: [ADR-0042](0042-in-memory-actors.md) (actor runtime), [ADR-0048](0048-single-homed-actors.md)
  ...
  [ADR-0050](0050-role-holder-proxy.md) (role-holder ingress forwarding), and
  [ADR-0062](0062-role-affine-routing-and-outbox-delivery.md) (general role affinity)
...
role leases named `actors:partition-0` through `actors:partition-15`. The process count is irrelevant to the
...
The partition is selected by the Coordination package's stable hash of the actor's logical name and
canonical key text. `ActorVirtualShard.GetShardIndex` remains actor-facing sugar over that primitive.
...
`SingleHome` and `VirtualShards` are placement alternatives. Generated `[ConsumeEvent]` descriptors
resolve the target role per event, and the outbox persists an independently claimable delivery for
that consumer (ADR-0062). The role holder claims it directly from PostgreSQL.
