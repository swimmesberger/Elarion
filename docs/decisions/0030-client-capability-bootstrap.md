# ADR-0030: Client capability bootstrap — modules, flags/variants, and grants projected to the frontend over OpenFeature
...
imperative `Map<…>(name, transports)` seam, and onto **REST** through a concrete, framework-authored
...
doesn't own; it has its own record in [ADR-0031](0031-imperative-handler-transport-mapping.md). The handler composes
...
exposed imperatively instead — see [ADR-0031](0031-imperative-handler-transport-mapping.md).
