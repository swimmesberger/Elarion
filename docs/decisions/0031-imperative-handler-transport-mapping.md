...
- a **framework-shipped** handler (e.g. the client-capability bootstrap of [ADR-0030](0030-client-capability-bootstrap.md));
...
owning the class. A naïve answer is a single generic `Map<TRequest,TResponse>(route, transports)`. That is
...
from attributes. So the existing generic, fluent `Map` seam is reused wherever the dispatcher is composed:

```csharp
dispatcher.Map<TRequest, TResponse>("elarion.session", HandlerTransports.JsonRpc);
```
...
resolver), a bus contribution via `Map<…>` (chained into the host's `RegisterHandlers`), and a concrete
`MapElarionX(route)` for REST. The host opts in and chooses which surfaces it wants. [ADR-0030](0030-client-capability-bootstrap.md)'s
...
- `HandlerDispatcher.Map<TRequest, TResponse>(string name, HandlerTransports transports)` in
  `Elarion.Abstractions.Dispatch` (fluent, returns the dispatcher) — the existing DI-resolved registration the
  generated bootstrapper already uses, now **also documented and reused as the host-facing imperative seam**, kept
  under its existing name (no rename).
- Framework capability extensions author their own concrete REST `Map*` method; **no** generic HTTP mapping API is
  provided, by design. [ADR-0030](0030-client-capability-bootstrap.md)'s `MapElarionSession` is the first such
  extension.
