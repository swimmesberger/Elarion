...
Write an `IEntityTypeConfiguration<T>` for the entity and mark it with `[EntityConfiguration]`, declare
the context interface with `[GenerateDbSets]`, and let the
...
```csharp title="MyApp.Application/Data/IAppDbContext.cs"
using Microsoft.EntityFrameworkCore;
using Elarion.EntityFrameworkCore;
