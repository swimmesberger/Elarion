...
[EF Core generator](/docs/entity-framework) emit the `DbSet`s.
...
### Add the host bootstrapper and RPC map

In the host, two small partial classes tell the generators to emit the cross-module wiring.
...
```csharp title="MyApp.Api/Rpc/RpcMethodMap.cs"
...
builder.Services.AddSingleton(
    RpcMethodMap.RegisterAll(new JsonRpcDispatcher(serializerOptions)).Freeze());
...
<Cards>
  ...
  <Card title="TypeScript client" href="/docs/json-rpc/typescript-client">
    Export a schema and generate a typed frontend client for `clients.get`.
  </Card>
  <Card title="Scheduled jobs" href="/docs/scheduling">
