...
Each module owns the schema for the entities it works with. Put the `IEntityTypeConfiguration<T>` in
the module, beside its handlers — it references only the shared-kernel entity, so it never crosses a
module boundary (and ELMOD002 only inspects constructor/field/property surface, never the `Configure`
body). The generator discovers these wherever they live and emits direct `Configure(...)` calls — no
