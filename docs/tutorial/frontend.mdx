...
<Callout type="info">
  ...
  capturing the API resource as `var api = builder.AddProject<Projects.Billing_Api>("api")…` (package `Aspire.Hosting.JavaScript`). Then `dotnet run` on the app host starts PostgreSQL, the API,
