...
The project layout is your choice: the bootstrapper discovers handlers both from referenced module
assemblies and from the host compilation itself, so a single-project app wires identically. This
tutorial uses the multi-project split because it is the shape that grows well.
...
// The unit of work behind Elarion.Pipeline.TransactionDecorator (and the inbox/idempotency decorators):
// every transactional handler begins, commits, and rolls back on the billing context.
...
// Handler authorization: the IAuthorizer that the generated decorator consults for [RequirePermission].
...
`MapElarionEndpoints` publishes every handler that carries `[HttpEndpoint]` as a minimal-API REST
route — the same handler, pipeline, and `Result` mapping as JSON-RPC, just a different projection.
None of the Billing handlers opt in so far; making one RESTful is a single attribute:
...
See [HTTP endpoints](/docs/capabilities/transports/http-endpoints) for routing, binding, and verb rules.
...
<ItemGroup>
  <PackageReference Include="Elarion.AspNetCore.SchemaGeneration" Version="0.2.6" PrivateAssets="all" />
</ItemGroup>
