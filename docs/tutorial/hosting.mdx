...
One rule the compiler will not enforce for you: the bootstrapper composes these maps from
**referenced assemblies' manifests**, so handlers declared in the host project itself never reach
them — a single-project app compiles and then silently 404s every route. Keep modules in the
referenced application project (the split this tutorial uses), even when the app is tiny.
