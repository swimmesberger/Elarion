...
<Callout type="info">
  ...
  export — so the generated TypeScript types match exactly what the server serializes. JSON-RPC and MCP are
  thin adapters over one handler bus, so you could expose MCP without ever calling `MapElarionJsonRpc()`.
</Callout>
