...
### Add the generated host partials
...
```csharp title="src/Billing.Api/Rpc/RpcMethodMap.cs"
using Elarion.JsonRpc;

namespace Billing.Api.Rpc;
...
`ModuleBootstrapper` gains `ConfigureAllServices`, `MapAllEndpoints`, and
`GetAllJsonTypeInfoResolvers`. `RpcMethodMap` gains `RegisterAll` (every `[RpcMethod]` handler) and
`McpMetadata()` (the tool table for MCP).
...
// Scheduler: descriptors (generated) + the in-memory runtime.
builder.Services.AddBillingApplicationScheduledJobs();
...
// Compose every module's services (handlers, services, validators).
ModuleBootstrapper.ConfigureAllServices(builder.Services, builder.Configuration);

// JSON-RPC: one serializer for runtime dispatch and schema export.
...
// MCP: one tool per [RpcMethod] handler, sharing the same dispatcher.
builder.Services.AddElarionMcp(
    RpcMethodMap.McpMetadata(), serializerOptions, o => o.ServerName = "Billing");
...
<Callout type="info">
  The **same** `serializerOptions` instance flows into the dispatcher, the MCP server, and schema
  export — so the generated TypeScript types match exactly what the server serializes. `MapElarionMcp()`
  only needs the dispatcher singleton; you could expose MCP without ever calling `MapJsonRpc()`.
</Callout>
...
Wire the [build-time schema exporter](/docs/json-rpc/schema-generation) so `rpc-schema.json` is written
