...
`ModuleBootstrapper` gains `AddElarion`, `MapElarionEndpoints`, `RegisterRpcMethods` (JSON-RPC),
...
// Infrastructure capabilities: concrete adapters behind the application's intent-only ports — the email
// sender and the cross-cutting audit trail (a port outside every module), wired here like any adapter.
...
<Callout type="info">
  ...
  dispatcher, so you could expose it without ever calling `MapElarionJsonRpc()`.
</Callout>
