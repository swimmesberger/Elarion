...
public sealed class ValidationDecorator<TRequest, TResponse>(
    ...
    public async ValueTask<TResponse> HandleAsync(TRequest request, CancellationToken ct) {
        ...
        // Both Result<T> and the non-generic Result implement IResultFailureFactory<TSelf>; the constraint
        // lets us build the failure via the static TResponse.Failure(error) — no reflection — and scopes the
        // decorator at compile time to Result-returning handlers.
