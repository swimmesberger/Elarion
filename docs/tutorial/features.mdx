...
### Add the audit trail (a Core module capability)

Command handlers should record *who* did *what* — and audit history is something users **read back**, so
those records are **queryable domain data**, not a write-only log. That makes audit a domain capability the
...
beside the others (it drives the generated `DbSet<AuditEntry>`):

```csharp title="src/Billing.Application/Domain/AuditEntry.cs"
...
```csharp title="src/Billing.Application/Modules/Core/Contracts/IAuditTrail.cs"
...
`[Service(typeof(IAuditTrail))]` registers the internal impl against the contract in Core's generated
`ConfigureDefaultServices` — no host wiring. `Clients` and `Invoicing` inject `IAuditTrail` (a contract, so
`ELMOD002` allows the cross-module dependency) and call `RecordAsync`; the entry persists in the same
transaction as the command. If you later back audit onto Grafana instead, that goes behind its *own* port
