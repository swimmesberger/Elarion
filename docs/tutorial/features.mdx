...
### Add the audit trail (a platform-capability port)

Command handlers should record *who* did *what*. Audit is **intent-only** — callers just say "record this
happened," and the app never queries audit records as domain — so it is a platform-capability **port** that
lives *outside* every module, with its adapter in `Billing.Infrastructure` (exactly like the email sender
later). Putting the port outside the modules lets any module depend on it without tripping the
[module boundary analyzer](/docs/concepts/cross-module-communication); a `[ModuleContract]` would be the
wrong tool here — that is for cross-module *domain* calls, not shared infrastructure.

```csharp title="src/Billing.Application/Abstractions/IAuditTrail.cs"
...
The adapter injects [`ICurrentUser`](/docs/capabilities/current-user) — a transport-neutral view of the
caller, so application code never touches `HttpContext`:

```csharp title="src/Billing.Infrastructure/Audit/AuditTrail.cs"
...
It just logs here, but it could write its own audit table and still be infrastructure — the
[logic/infra line](/docs/why-elarion) is domain-data vs. mechanism-state, not database vs. no-database (the
EF Core outbox owns a table the same way). The host registers it like any other intent-only adapter, which
you do in [Host the API](/docs/tutorial/hosting):
