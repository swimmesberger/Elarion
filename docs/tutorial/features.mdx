...
conventions. Define them in the application and expose named pipelines with `[DecoratorList]`.
...
Now expose two named pipelines. The **default** applies to every command; the **read-only** pipeline
skips the transaction for queries.
...
handler. Apply the default across the whole assembly:
...
that injects [`ICurrentUser`](/docs/concepts/current-user) — a transport-neutral view of the caller, so
...
through to the [MCP tool](/docs/json-rpc/mcp) surface.
...
Reads are `Query` types on the read-only pipeline (no transaction). `GetClient` is marked
`[Cacheable]`: the generator inserts a cache decorator that keys off the request and only runs the
handler on a miss. The default `CurrentUser` scope isolates entries per account, so one user's cached
...
Each module contributes [source-generated JSON metadata](/docs/json-rpc/serialization) for its
