...
| `EventConsumerRegistrationGenerator` | `[assembly: UseElarion]` or `[assembly: GenerateEventConsumers]` | Per-module `Add{Module}EventConsumers()` registering each `[ConsumeEvent]` service plus its `EventSubscriptionDescriptor`. See [Events](/docs/events). |
