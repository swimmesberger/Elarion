...
- `Elarion.AspNetCore` — ASP.NET Core JSON-RPC endpoint mapping, batch execution, current-user middleware (the snapshot exposes `ICurrentUser` claim access), HTTP transport support, and minimal-API endpoint mapping for `[HttpEndpoint]` handlers (`HttpAppErrorMapper` maps `Unauthorized`→401/`Forbidden`→403, `ElarionHttpResults`). Also the concrete REST `MapElarionSession(route)` for the client-capability bootstrap (concrete because ASP.NET's RDG needs it for AOT — see [Client capability bootstrap](#client-capability-bootstrap)).
...
- `@swimmesberger/elarion-jsonrpc-client-generator` — TypeScript CLI/library that converts exported Elarion JSON-RPC schemas into method contracts, Zod result schemas, and a portable fetch client. When the schema exposes the `elarion.session` operation it also emits a self-contained `session-client.ts` — a typed `ClientSnapshot`, `SessionCapabilities`, typed `Keys`, and an OpenFeature web-SDK provider hydrated from the snapshot (see [Client capability bootstrap](#client-capability-bootstrap)). Lives in `src/elarion-jsonrpc-client-generator`.
...
transports (JSON-RPC, MCP) can route a wire *string* to a handler. The bus is populated by
`dispatcher.MapHandler<TRequest, TResponse>(name, transports)` — one call per `[Handler]` from the generated
bootstrapper, and the **imperative exposure seam** a host uses to map a handler whose class it does not own (a
framework- or third-party-shipped handler) onto the name-routed transports (REST stays concrete — see
[Client capability bootstrap](#client-capability-bootstrap) and ADR-0021). **In-process, always call handlers
...
A **framework-shipped** handler exposes the whole client picture for the current user and deployment to the frontend
in one snapshot — which modules are enabled, the exposed flags/variants, and the user's roles/permissions — so a UI
can hide or adapt itself. It is a **read-only UX projection, not enforcement** (the handler's `[RequirePermission]`/
`[FeatureGate]` is still the real gate). See [ADR-0020](docs/decisions/0020-client-capability-bootstrap.md),
[ADR-0021](docs/decisions/0021-imperative-handler-transport-mapping.md), and
[the client-capabilities concept doc](docs/concepts/client-capabilities.mdx).

1. A module declares the names it exposes with `[ClientFeatures("a", "b")]` on its `[AppModule]` type (in
   `Elarion.Abstractions.Modules`). Exposure is **opt-in by enumeration** — nothing reaches the wire unless a module
   names it — and a listed name needs **no** `[FeatureGate]`/`[FeatureVariant]` behind it (a pure UI flag is
   first-class). `AppModuleDiscoveryGenerator` collects the names (round-tripped cross-assembly through the Elarion
   manifest) into a gated `configuration.GetClientCapabilityManifest()` on `ElarionBootstrapper`, emitted only when a
   module opts in.
2. The `SessionHandler` (`Elarion` core, namespace `Elarion.Session`) composes `GetClientCapabilityManifest` +
   `IFeatureFlagService` + `IFeatureVariantService` + `ICurrentUser` into a `SessionResponse`
   (`{ user{id,email?,isAuthenticated,roles,permissions}, modules, flags, variants }`). Only enabled modules' names
   are evaluated; a name appears under `variants` when the variant accessor resolves one, else under `flags`. The
   flag/variant services are **optional**, so a host without feature flags still gets modules + grants.
3. Because the handler is framework-owned (no `[Handler]` attribute), it is exposed **imperatively** (ADR-0021):
   `AddElarionSession(manifest)` (DI), the bus `MapElarionSession()` (a `MapHandler` over `"elarion.session"`, chained
   into the host's `RegisterHandlers` so the shared dispatcher is still built once), the concrete REST
   `MapElarionSession(route)` in `Elarion.AspNetCore` (concrete because ASP.NET's RDG needs it for AOT — no generic
   HTTP map), and the `SessionJsonContext` AOT resolver the host combines into its `JsonSerializerOptions`.
4. On the client, when the schema exposes `elarion.session`, the TypeScript generator emits a self-contained
   `session-client.ts`: a typed `ClientSnapshot`, `SessionCapabilities` accessors, the typed `Keys`, and an
   **OpenFeature web-SDK provider** hydrated from one snapshot that answers every key from the cache via the reserved
   `module.`/`permission.`/`role.` namespaces (a bare key is a flag/variant) — so the frontend uses one mechanism.
