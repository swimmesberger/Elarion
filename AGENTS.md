...
4. The default `IFeatureFlagService` targets **OpenFeature** (the vendor-neutral standard), so `[FeatureGate]`
   works against any OpenFeature provider. `Elarion.OpenFeature` (`AddElarionOpenFeature`, bring your own
   provider) is the base; `Elarion.FeatureManagement` (`AddElarionFeatureManagement(config)`) is the
