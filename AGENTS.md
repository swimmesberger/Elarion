...
- `Elarion.Resilience` — Polly-backed `IResiliencePipelineRunner` default (`AddElarionResilience`); pulls `M.E.Resilience`. Catalog/metadata + `ResilienceDecorator` stay in core/Abstractions (ADR-0017).
...
- `Elarion.Blobs` — provider-neutral, streaming-first blob contracts: `IBlobStore` (`SaveAsync`/`OpenReadAsync`→disposable `BlobDownload`/metadata/delete/exists), ergonomic `byte[]`/file helpers as extensions, and the **upload lifecycle** (`BlobLifecycleState` Pending/Committed, `IBlobLifecycle` — `CommitAsync` inside the caller's tx, `DeleteExpiredPendingAsync` GC). Stays S3-free.
- `Elarion.Blobs.PostgreSql` — Postgres blob storage via EF model config + Npgsql streaming I/O (`AddElarionPostgreSqlBlobStore<T>`; `UseElarionBlobStorage`/`[GenerateElarionBlobStorage]`, ELBLB001). Implements `IBlobLifecycle` (partial index over pending rows) + `BlobGarbageCollector`.
- `Elarion.Blobs.AspNetCore` — minimal direct upload endpoint (`MapElarionBlobUploads`: POST bytes→Pending `BlobRef`, DELETE→owner cancel); content-type allow-list + size cap. Fits FilePond/`fetch`.
- `Elarion.Blobs.Tus` — **tus 1.0** resumable upload transport (`MapElarionTus`); the recommended transport (Uppy/`tus-js-client`). Completed upload → Pending blob, ref in `Elarion-Blob-Ref`. `ITusUploadStore` seam; default in-memory.
- `Elarion.Blobs.Tus.PostgreSql` — durable `ITusUploadStore` staging bytes in a `tus_uploads` column (survives restart) (`AddElarionTusPostgreSql<T>`; `UseElarionTusStorage`/`[GenerateElarionTusStorage]`, ELTUS001) + reaper.
...
- `Elarion.AspNetCore` — ASP.NET JSON-RPC endpoint mapping, batch, current-user middleware, `[HttpEndpoint]` minimal-API mapping (`HttpAppErrorMapper`, `ElarionHttpResults`), and `AddElarionHttpJson()` (mirrors canonical JSON onto `Http.Json.JsonOptions` — needed under reflection-off).
...
- `Elarion.Messaging.Outbox` — EF transactional outbox: durable `IIntegrationEventBus` recording each event as an `OutboxMessage` (atomic with business data) + hosted delivery loop (conditional `ExecuteUpdate` lease, at-least-once) (`AddElarionOutbox<T>`; `UseElarionOutbox`/`[GenerateElarionOutbox]`, ELOBX001; partial index over pending rows). `IOutboxStore` seam.
- `Elarion.EntityFrameworkCore.UnitOfWork` — framework EF transaction boundary: `EfUnitOfWork<T>` over the EF-free `IUnitOfWork` seam (PostgreSQL `SET LOCAL lock_timeout` + savepoints) (`AddElarionUnitOfWork<T>`). The framework `TransactionDecorator` (`AppliesTo` = `ICommand`/`IIntegrationEvent`, not `[Idempotent]`) composes it (ADR-0021).
- `Elarion.Idempotency.EntityFrameworkCore` — durable idempotency store: `INSERT … ON CONFLICT DO NOTHING` **inside the caller's tx** (composite-PK constraint is the cross-node fence, no external lock; `55P03` lock_timeout → 409) (`AddElarionIdempotencyEntityFrameworkCore<T>`; `ApplyElarionIdempotencyKeys`/`[GenerateElarionIdempotencyKeys]`, ELIDEMEF001). `[Idempotent]` + seams + in-memory default live in Abstractions/`Elarion` (ADR-0021).
- `Elarion.Generators` — Roslyn generators (handlers, services, modules, RPC/HTTP/MCP maps via `AppModuleDiscoveryGenerator`, validation resolvers, resilience policies, scheduled jobs, event consumers, permission catalog, per-module `ConfigureDefaultServices`). `HandlerRegistrationGenerator` auto-attaches the authorization → feature-gate → validation decorators (see the model sections). Hosts `ModuleApiGenerator` + `ModuleBoundaryAnalyzer` (ELMOD002). **`IsPackable=false`** — bundled into `Elarion`.
- `Elarion.EntityFrameworkCore.Generators` — EF generators: `DbContextGenerator` (DbSets + `ConfigureEntities` + the `OnEntitiesConfigured` seam other feature generators implement; cross-assembly via `EntityConfigurationManifest`), the `[Keyset]` emitter (provider-aware row-value seek under Npgsql), and `[ResourceFilter]` (ELKEY005). **`IsPackable=false`** — bundled into `Elarion.EntityFrameworkCore`.
- `@swimmesberger/elarion-jsonrpc-client-generator` — TS CLI/library: schema → method contracts, constraint-aware Zod params/result schemas, a params-pre-validating fetch client (`validateParams:false` opts out). Zod v3+v4. `src/elarion-jsonrpc-client-generator`.
- `@swimmesberger/elarion-contributions` — the **frontend contribution model** (ADR-0032): typed extension-point tokens (`defineExtensionPoint<TItem,TContext>` — the frontend `[ModuleContract]`), declarative module manifests (`defineModule` + `contribute(point, items)`; manifest-level `when` ANDs into every item), the `when` evaluator (`{module?,permission?,flag?,role?}`, AND, fail-closed — **UX projection, never security**), the deterministic `createContributionRegistry`, and `createContributionKit<Vocabulary>()`. Zero-dep core; `/react` bindings (`ContributionProvider`/`useContributions`/`<ExtensionSlot>`, React optional peer); `/tanstack-router` ships one helper — `redirectUnless(when, to)`/`createRouteGuards<V>()` — and no other routing machinery. Point payload shapes are app-owned (no framework `NavItem` — rejected as not framework-worthy; the modular-sidebar/shadcn recipe is in [frontend-modules](docs/concepts/frontend-modules.mdx)). Recommended composition root discovers modules via `import.meta.glob("./modules/*/index.ts", {eager:true, import:"default"})` (a new module is a new folder; cost: glob-typed route tree loses `Link to` literal checking — a static tuple is the typed alternative). `src/elarion-contributions`.
...
- `Elarion` core is **dependency-light** (only Abstractions + `M.E.*` *Abstractions*) and **transport-agnostic** (no protocol/host/ASP.NET/EF package, not even `Elarion.JsonRpc`). A heavy provider default lives in its own opt-in sibling; the seam + pipeline decorator stay in Abstractions (ADR-0017). Do not reintroduce a concrete third-party runtime dep into core/Abstractions.
...
- `Elarion.Blobs` stays provider-neutral and S3-free (lifecycle in core; upload protocols are HTTP-layer sources). The S3 wire protocol is a deliberate non-goal.
- Authorization, feature-flag gating, and request validation each follow the same **seam/impl split**: attribute + seam + decorator in Abstractions (no provider dep); the heavy default in an opt-in package. Swapping a provider must never touch a `[FeatureGate]`/`[Require*]`/DTO. Validation uses standard `System.ComponentModel.DataAnnotations` — **no Elarion validation attribute, no FluentValidation dep anywhere** (app-owned decorator only). Business rules live in handlers, never a pre-handler validator (ADR-0027).
...
reads `[Authorize]`/`[AllowAnonymous]`. RPC/MCP gating needs `IConfiguration` at compose time — use the config-aware
...
- **Frontend contributions** (`src/elarion-contributions`, ADR-0032): the review-isolation model for the frontend — see the
  ...
  kernel + `/react` bindings + the one `/tanstack-router` guard.
...
- DB integration tests use Testcontainers behind a Docker-gated fixture (e.g. `PostgreSqlBlobStoreFixture`): spin up a real
  container when Docker is available, **skip** (never fail) when not, so `dotnet test` stays green Docker-free. Tag
