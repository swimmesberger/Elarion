...
- `Elarion.Blobs.AspNetCore` — direct blob-transfer endpoints: upload (`MapElarionBlobUploads`: POST bytes→Pending `BlobRef`, DELETE→owner cancel; content-type allow-list + size cap, fits FilePond/`fetch`) and the owner-scoped streaming download (`MapElarionBlobDownloads`: GET /{blobId}, exact-owner fail-closed → 404; the staged-blob export leg, ADR-0038).
...
- `Elarion.AspNetCore` — ASP.NET JSON-RPC endpoint mapping, batch, current-user middleware, `[HttpEndpoint]` minimal-API mapping (`HttpAppErrorMapper`, `ElarionHttpResults` — incl. `ToFileResult` for `Result<ElarionFile>` downloads, ADR-0038), `[ModuleEndpoints("Name")]` (endpoint hooks for a module declared outside its assembly — host or web companion, called inside the module's feature gate; ELMOD004/ELMOD005, ADR-0039), and `AddElarionHttpJson()` (mirrors canonical JSON onto `Http.Json.JsonOptions` — needed under reflection-off).
...
- `@swimmesberger/elarion-jsonrpc-client-generator` — TS CLI/library: schema → method contracts, constraint-aware Zod params/result schemas, a params-pre-validating fetch client (`validateParams:false` opts out), and native `File` mapping for `x-elarion-file` payloads (params take a `File`, results materialize one; base64 conversion at the call boundary, file-free schemas byte-identical). Zod v3+v4. `src/elarion-jsonrpc-client-generator`.
...
- **HTTP/REST** is a separate opt-in (`[HttpEndpoint]`) because it needs route/verb/param binding. Verb precedence:
  ...
  **Files are two-tiered** (ADR-0038). Small (≲4 MB): `ElarionFile` in the request/response — HTTP maps
...
unknown module → `ELMOD004`, hook-less class → `ELMOD005` (ADR-0039). RPC/MCP gating needs `IConfiguration` at compose time — use the config-aware
