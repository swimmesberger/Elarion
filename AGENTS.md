...
- `Elarion.Blobs.Tus` — **tus 1.0** resumable upload transport (`MapElarionTus`); the recommended transport (Uppy/`tus-js-client`). A pure protocol adapter over `IStagedUploadStore` (all policy in `TusOptions`, handed to the store as instants); completed upload → Pending blob, ref in `Elarion-Blob-Ref`.
