...
5. **Variant service injection** (ADR-0018) ships a different *implementation* per allocated variant. The
   consuming handler is transparent (injects the contract like any service); only the implementations carry
   `[FeatureVariant<TContract>("feature", Variant="x")]`. Because variant selection is an async flag evaluation
   ...
   escape hatch (`ELVAR001`–`ELVAR007`). `[FeatureVariant]` is a **modifier on `[Service]`**, not a separate
   registration: the impl must also carry `[Service]` (which declares the service + lifetime; a `[FeatureVariant]`
   without `[Service]` is `ELVAR007`), and `ModuleServiceRegistrationGenerator` skips the plain registration for a
   `[FeatureVariant]` class so the keyed + transparent registration is the only one. So `[Service]` stays the one
   consistent way to register a service — the modifier just makes it variant-keyed. The recommended way to vary
