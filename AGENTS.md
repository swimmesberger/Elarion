...
- `Elarion.JsonRpc` — the JSON-RPC transport: the dispatcher, envelopes, result/error types, telemetry, and schema export, **plus the Elarion handler bridge** — `MapHandler` (maps `IHandler<,>` onto the dispatcher), `AppErrorMapper`, and the default `IAppErrorTranslator<RpcError>`. References `Elarion.Abstractions` (for the handler/result/error contracts and the dispatch-scope rail) but stays ASP.NET-free. The transport-neutral dispatch-scope rail itself lives in `Elarion.Abstractions`, not here.
...
- `Elarion` depends only on `Elarion.Abstractions` and is **transport-agnostic**: it must not reference any transport package (including `Elarion.JsonRpc`), ASP.NET Core, or EF Core. Transports are sibling packages layered above core (`Elarion.JsonRpc`, `Elarion.AspNetCore`, `Elarion.AspNetCore.Mcp`).
