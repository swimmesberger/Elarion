...
- `Elarion.Messaging.Outbox` — EF transactional outbox (ADR-0062): immutable `OutboxMessage` plus one independently leased/retried `OutboxDelivery` per consumer, inserted atomically with business data. Generated descriptors supply stable consumer ids and optional target roles; workers claim unbound deliveries or those targeting a locally held role, so `SingleHome`/`VirtualShards` actor consumers follow placement without HTTP forwarding. Per-consumer finalize prevents sibling failures from replaying completed siblings; `OutboxMessage.Id` remains the inbox key. Publishing requires the complete consumer catalog; `RunDeliveryWorker=false` disables only claiming. `UseElarionOutbox`/`[GenerateElarionOutbox]` map/generate both tables.
