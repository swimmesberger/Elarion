...
`ActorPlacementMode.Local` (default), `SingleHome`, or `VirtualShards` (ADR-0060). The old boolean
...
- `Elarion.Actors.PostgreSql` — PostgreSQL actor snapshot store (ADR-0047) + actor home lease (ADR-0048): `elarion_actor_snapshots` (jsonb payload — actor state stays SQL-inspectable; `version bigint` = the ETag) behind `IActorSnapshotStore` + the `IActorStateReader` query companion (`AddElarionPostgreSqlActorSnapshots<T>`; `UseElarionActorSnapshots`/`[GenerateElarionActorSnapshots]`, ELASN001). Singleton store, fresh DI scope per operation (actor turns run outside handler scopes — never pins a `DbContext` to an activation); change-tracker-free writes (`INSERT … ON CONFLICT DO NOTHING` create, version-guarded `ExecuteUpdate`/`ExecuteDelete`). **Actor home** (ADR-0048): `AddElarionPostgreSqlActorHome<T>` = sugar over the `"actors"` role lease (ADR-0049) + `AddElarionActorHome(role)` (the `IActorHomeLease` view in `Elarion.Actors`) electing the instance that runs `[Actor(Placement = ActorPlacementMode.SingleHome)]` actors; calls elsewhere fail with `ActorNotHomedException` (no forwarding — that's the Orleans trigger); unenforced (warned) without a registered lease; event delivery follows via `OutboxOptions.DeliveryGate`, host-composed. Not `IsAotCompatible`.
