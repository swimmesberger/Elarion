...
- `Elarion.Resilience` — the Microsoft/Polly-backed `IResiliencePipelineRunner` default (`MicrosoftResiliencePipelineRunner` + `AddMicrosoftResilienceRuntime`, namespace `Elarion.Resilience`). Opt-in: referencing it pulls `Microsoft.Extensions.Resilience`. The lightweight policy catalog/metadata and the `ResilienceDecorator` stay in core/`Elarion.Abstractions`; `AddInMemoryScheduler` registers the catalog, so basic scheduling needs no resilience package, while `[Resilient]` handlers and deferred scheduler retries opt into the runner (see [ADR-0017](docs/decisions/0017-dependency-light-core.md)).
...
- `Elarion.AspNetCore.OpenApi` is the only package that references `Microsoft.AspNetCore.OpenApi`; the base HTTP transport and the generator stay OpenAPI-free. The generator's `.WithTags("{Module}")` and the inert `ElarionIdempotentEndpointMetadata` marker it emits for `[Idempotent]` `[HttpEndpoint]` handlers are ignored by a host that does not reference the OpenAPI package. Do not add an Elarion OpenAPI MSBuild package or a bespoke HTTP client generator — build-time export reuses `Microsoft.Extensions.ApiDescription.Server` and client generation is off-the-shelf (ADR-0024).
...
[ADR-0024](docs/decisions/0024-openapi-http-transport.md)). It is deliberately thin — no bespoke generator, no
...
`AddInMemoryIntegrationEventBus<TContext>` (or `AddInMemoryEventBus<TContext>` for both): `InMemoryIntegrationEventBus`
...
non-generic `AddInMemoryIntegrationEventBus` registers the building blocks without attaching them, for
