...
5. Every `[RequirePermission]`/`[RequireRole]` string is surfaced as a catalog so seeding and role→permission
   policy enumerate the full set instead of a hand-kept `Permissions.All` list (zero central edits per guarded
   handler). `PermissionCatalogGenerator` emits two surfaces: the **compile-time** `ElarionPermissions` static
   (in the assembly's root namespace — a non-module namespace, so static role policy referencing it never trips
   `ELMOD002`) with `All`/`Roles`/`ByModule`/`ByKind` and typed accessors (`ElarionPermissions.Users.Manage`),
   aggregated cross-assembly from the Elarion manifest; and the **runtime** `IPermissionCatalog`
   (`Elarion.Abstractions.Authorization`, registered by `AddElarionAuthorization`) aggregating one
   `PermissionCatalogModule` per module through the module's gated `ConfigureDefaultServices` (so it spans
   referenced module assemblies and a disabled module contributes nothing). The optional
   `[RequirePermission("x", PermissionKind.Read)]` classification drives `ByKind` (no `.read`-suffix convention);
   the generator resolves a `const`-referenced permission to its value. Trigger
   `[UseElarion]` / `[GeneratePermissionCatalog]`; a guarded handler under no module → `ELPERM001`, a colliding
   typed accessor → `ELPERM002`.
