...
- `Elarion.Generators` — Roslyn generators (handlers, services, modules, RPC/HTTP/MCP maps via `AppModuleDiscoveryGenerator`, validation resolvers, resilience policies, scheduled jobs, event consumers, permission catalog, per-module `ConfigureDefaultServices`). `HandlerRegistrationGenerator` auto-attaches the authorization → feature-gate → validation decorators (see the model sections). Hosts `ModuleApiGenerator` + `ModuleBoundaryAnalyzer` (ELMOD002) + `PreferGuidCreateVersion7Analyzer` (ELID001: flag `Guid.NewGuid()`, prefer `Guid.CreateVersion7()`; silent below .NET 9, ADR-0038). **`IsPackable=false`** — bundled into `Elarion`.
- `Elarion.Generators.CodeFixes` — the Workspaces-layer code-fix sibling (ELID001 rewrite). A separate assembly because the compiler loads analyzers without Workspaces (RS1038); bundled into `Elarion`'s `analyzers/dotnet/cs` beside the generator. **`IsPackable=false`**.
...
- Mint `Guid` ids with `Guid.CreateVersion7()`, never `Guid.NewGuid()` (ELID001; entities own their ids —
  the model declares Guid PKs `ValueGeneratedNever`, ADR-0038). v4 only where unpredictability is required,
