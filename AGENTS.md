...
1. **Tier 1** is standard `System.ComponentModel.DataAnnotations` attributes on the request DTO —
   ...
   `[EmailAddress]`, `[Url]`, `[Base64String]` — enforced at runtime **and** exported to the JSON-RPC
   schema, MCP tool schemas, the OpenAPI document, and the generated Zod client (which pre-validates
   params). Requiredness comes from NRT + the `required` modifier (no `[Required]`). Reusable custom
...
6. Schema export: `JsonRpcSchemaExporter` injects the constraint keywords (`[Range]` →
   ...
   `[Base64String]` → `format: "byte"`; `[EmailAddress]` → `format: "email"`); MCP tool input schemas
   share the builder. OpenAPI gets the same set natively from Microsoft (reflection-off), plus an
   email-format parity transformer in `Elarion.AspNetCore.OpenApi`. The TS client maps the keywords to
   Zod and pre-validates params by default (`validateParams: false` opts out; local failures throw
