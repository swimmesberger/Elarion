...
5. Every `[RequirePermission]`/`[RequireRole]` string is also surfaced as a typed `IPermissionCatalog`
   (`Elarion.Abstractions.Authorization`), so seeding and role→permission admin enumerate the full set instead
   of a hand-kept `Permissions.All` list. `PermissionCatalogGenerator` discovers the strings, groups them by
   owning module, and registers one `PermissionCatalogModule` per module through the module's gated
   `ConfigureDefaultServices` (so it spans referenced module assemblies and a disabled module contributes
   nothing); the core catalog (registered by `AddElarionAuthorization`) aggregates them, deduplicated and
   sorted, with a per-module breakdown. Adding a guarded handler needs **zero** central edits. Trigger
   `[UseElarion]` / `[GeneratePermissionCatalog]`; a guarded handler under no module → `ELPERM001`.
