...
- `Elarion.Actors.PostgreSql` — PostgreSQL actor snapshot store (ADR-0047) + actor home lease (ADR-0048): `elarion_actor_snapshots` (jsonb payload — actor state stays SQL-inspectable; `version bigint` = the ETag) behind `IActorSnapshotStore` + the `IActorStateReader` query companion (`AddElarionPostgreSqlActorSnapshots<T>`; `UseElarionActorSnapshots`/`[GenerateElarionActorSnapshots]`, ELASN001). Singleton store, fresh DI scope per operation (actor turns run outside handler scopes — never pins a `DbContext` to an activation); change-tracker-free writes (`INSERT … ON CONFLICT DO NOTHING` create, version-guarded `ExecuteUpdate`/`ExecuteDelete`). **Actor home** (ADR-0048): `AddElarionPostgreSqlActorHome<T>` = sugar over the `"actors"` role lease (ADR-0049) + `AddElarionActorHome(role)` (the `IActorHomeLease` view in `Elarion.Actors`) electing the instance that runs `[Actor(SingleHomed = true)]` actors; calls elsewhere fail with `ActorNotHomedException` (no forwarding — that's the Orleans trigger); unenforced (warned) without a registered lease; event delivery follows via `OutboxOptions.DeliveryGate`, host-composed. Not `IsAotCompatible`.
- `Elarion.Coordination.PostgreSql` — PostgreSQL **role leases** (ADR-0049): heartbeat-renewed leader election for named roles ("exactly one instance is X") as one conditional-upsert row per role (`elarion_role_leases` — the row is the whole membership protocol; app clock only; `IsHeld` undershoots by `HeldSafetyMargin` so the old holder stops before a new one can start; release-on-shutdown = instant failover; DB outage → nobody holds, fail-closed). The row also carries the holder's **advertised address** (ADR-0050: `RoleLeaseOptions.AdvertisedAddress` or the `IInstanceAddressProvider` seam → `IRoleLease.CurrentHolderAddress`, heartbeat-fresh) — a hub, not a mesh. `AddElarionPostgreSqlRoleLease<T>(o => o.RoleName = …)` per role → keyed `IRoleLease` (contract in `Elarion.Abstractions.Coordination`) + heartbeat; duplicate role registration throws; `UseElarionRoleLeases`/`[GenerateElarionRoleLeases]`, ELROLE001. Consumers gate per call/cycle (no lease-change events). **Not a distributed-lock API** — coarse roles only; per-work-item = scheduler claims/outbox leases. Completes the coordination taxonomy (claims = per item, outbox leases = per message, role leases = per role). Not `IsAotCompatible`.
...
- `Elarion.AspNetCore` — ASP.NET JSON-RPC endpoint mapping, batch, current-user middleware, `[HttpEndpoint]` minimal-API mapping (`HttpAppErrorMapper`, `ElarionHttpResults` — incl. `ToFileResult` for `Result<ElarionFile>` downloads, ADR-0039), `[ModuleEndpoints("Name")]` (endpoint hooks for a module declared outside its assembly — host or web companion, called inside the module's feature gate; ELMOD004/ELMOD005, ADR-0040), and `AddElarionHttpJson()` (mirrors canonical JSON onto `Http.Json.JsonOptions` — needed under reflection-off). Also the **role-holder proxy** (ADR-0050): `UseElarionRoleHolderProxy(role, prefixes)` — every instance serves the listed prefixes by forwarding to the lease holder's advertised address BEFORE routing (no local execution → no double-execution; one hop, `Elarion-Role-Proxied` loop guard → 503+Retry-After; streams SSE; installs NOTHING when no lease is registered = zero single-instance cost; the prefix list is the future ingress rule) + `AddElarionInstanceAddress()` (the `IInstanceAddressProvider` default: server-bound endpoint, wildcard hosts → first non-loopback IPv4). Never exception-driven replay (double execution), never retries/queueing. Also the **ordered-stream SSE endpoint** (ADR-0052): `MapElarionStream<T>(route, (ctx, resumeAfter) => IAsyncEnumerable<StreamItem<T>>?)` — SSE `id:` = hub sequence, `Last-Event-ID`/`?after=` resume from the producer's ring, null → 404, auth = host's delegate/`.RequireAuthorization()`; streamed routes belong under the home-routed prefixes, `pg_notify` deliberately uninvolved.
...
- `Elarion.Messaging.Outbox` — EF transactional outbox: durable `IIntegrationEventBus` recording each event as an `OutboxMessage` (atomic with business data) + hosted delivery loop (conditional `ExecuteUpdate` lease, at-least-once; `RunDeliveryWorker=false` = publisher-only node — heterogeneous topologies run delivery only where **all** consumers are registered, else a node parks foreign messages as unresolvable; `DeliveryGate` = the dynamic sibling, consulted per cycle — gate on `IActorHomeLease.IsHeld` so delivery follows the actor home, ADR-0048) (`AddElarionOutbox<T>`; `UseElarionOutbox`/`[GenerateElarionOutbox]`, ELOBX001; partial index over pending rows). `IOutboxStore` seam. Seeds each delivery scope with `OutboxMessage.Id` (`IEventContext.MessageId`, the inbox key) and wires `AddElarionIdempotency` for the default-on inbox (ADR-0022).
- `Elarion.Migrations.PostgreSql` — SQL migration runner for **EF-free (NativeAOT) hosts only** (ADR-0057; EF tier keeps EF migrations — never a competing default; ships no framework-table scripts): embedded `V{version}__{description}.sql`/`R__{description}.sql` resources (timestamp versions recommended; version segments in file names use single underscores — resource names split on dots), fail-closed total discovery validation, SHA-256 checksums over BOM-stripped CRLF→LF-normalized content (line-ending churn never invalidates; genuine mismatch names script + both hashes + the two legal fixes). One dedicated connection, **session-level** `pg_advisory_lock` (never xact-scoped — the Flyway `CREATE INDEX CONCURRENTLY` hang); each versioned script's `elarion_schema_history` row commits **in the script's own transaction** — failed transactional migration = no row, rerun after fixing, **no repair command exists**. `-- elarion: no-transaction` (leading comment block; unknown directive = validation error) splits statement-by-statement via the built-in splitter (dollar-quote/comment/string-aware); only such a failure records a failed row (versioned scripts only; a failed repeatable records nothing and reruns) → subsequent runs fail closed naming `ResolveFailedAsync(version, Retry \| MarkApplied)`. Out-of-order default `Warn` (applied + recorded in true `installed_rank` order; `Deny` opt-in); explicit `BaselineAsync` only (empty history); undo/placeholders/dialects/tiering rejected — replace the tool (Flyway in CI) rather than grow it (ADR-0025). `AddElarionPostgreSqlMigrations(connectionString \| dataSource, o => o.AddScripts(assembly, prefix))` = runner + migrate-before-ready hosted service (`ApplyOnStartup=false` opts out). Contracts live in the package (no Abstractions seam). Deps: `Npgsql` + `M.E.*` abstractions; `IsAotCompatible`.
- `Elarion.Sql` — AOT-native SQL row mapping for EF-free hosts (ADR-0058), the access half of the AOT tier (ADR-0057 is the schema half; EF tier untouched — chosen by tier, not preference): `[SqlRecord]` → generated sealed `{Type}SqlMapper : ISqlRowMapper<T>` (bundled generator, ADR-0006-conformant) with `Ordinals` resolved by name once per result set then synchronous typed `GetFieldValue<T>` reads (no name lookups, no boxing, no per-column await), typed `BindParameters` (params named like columns), `TableName`/`Columns.*`/`Columns.All`/`Columns.AllParameters` constants, static `Instance` (JSON mappers instead take `IElarionJsonSerialization` — `[SqlJson]` rides the canonical accessor, ADR-0023, `jsonb` under `[assembly: UseElarionSql(Provider = SqlProvider.Npgsql)]`), and a per-assembly `AddElarionSqlMappers` registration. `[SqlColumn]`/`[SqlIgnore]` per property; snake_case default both names. **No interception, no reflection twin — unmapped type = compile error** (`ELSQL001`–`ELSQL007`), the deliberate inversion of Dapper.AOT's call-site interception (its indirection fallback is architectural, disqualifying for AOT-first). Positional records construct through the primary ctor; nominal `required`/`init` records via object initializer. Full-power-of-SQL surface: `SqlStatement.Of($"…")` interpolation binds values as `@pN` parameters at compile time (interpolated string handler; collections expand for `IN`, **empty collections throw** — no spelling keeps IN and NOT IN both correct; `{expr:raw}` splices trusted identifiers; a `SqlStatement` hole splices as a composable fragment with renumbered params; deliberately NO `Of(string)` overload — constant-only interpolations would bind to it and splice instead of parameterize; plain text = `SqlStatement.Raw`), plus thin `DbConnection` extensions (`QueryAsync`/`QueryFirstOrDefaultAsync`/`ExecuteAsync`/`ExecuteScalarAsync` — Dapper open/close semantics). **Benchmarked at hand-written-ADO.NET parity** in time and allocations, with Dapper/Dapper.AOT/EF no-tracking columns (`*SqlMapping*` in `tests/Elarion.Benchmarks`, the ADR-0051 discipline). NOT an ORM: no LINQ, no change tracking, no CRUD statement gen, no query-builder DSL (rejected as LINQ-to-SQL-by-another-name). Schema-derived metamodel + drift verification (design-time snapshot tool → `elarion-sql-schema.json` AdditionalFile) is committed but sequenced after v1. Deps: `Elarion.Abstractions` + `M.E.DependencyInjection.Abstractions`; `IsAotCompatible`.
