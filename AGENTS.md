...
- `Elarion.Validation` — the `Microsoft.Extensions.Validation`-backed `IRequestValidator` default for declarative request validation (`MicrosoftRequestValidator` + `AddElarionValidation([Action<ValidationOptions>])`, which also calls `AddElarionJson`; the `AddElarionValidationResolver(resolver)` helper is what generated module code calls). Opt-in in the ADR-0017 shape: referencing it is what pulls `Microsoft.Extensions.Validation` 10.x — **runtime base only** (`ExcludeAssets="analyzers"`, Microsoft's bundled validation generator never runs; Elarion's `ValidationResolverGenerator` supplies the metadata instead) — keeping core dependency-light (`Microsoft.Extensions.Options` rides along, which is why the dependency is barred from core). Implements the seam over `ValidationOptions`/`IValidatableInfoResolver` + `ValidateContext`, translates error paths through the canonical JSON naming policy, and suppresses the `[Experimental("ASP0029")]` surface internally so the rest of the framework never touches it. The application project references it (that reference is what triggers per-module resolver generation and silences `ELVAL002`). `IsAotCompatible`. Depends on `Elarion.Abstractions` + `Microsoft.Extensions.Validation` (which pulls `Microsoft.Extensions.DependencyInjection.Abstractions` + `Microsoft.Extensions.Options`). See [Validation model](#validation-model) and [ADR-0027](docs/decisions/0027-declarative-request-validation.md).
...
- `Elarion.AspNetCore` — ASP.NET Core JSON-RPC endpoint mapping, batch execution, current-user middleware (the snapshot exposes `ICurrentUser` claim access), HTTP transport support, and minimal-API endpoint mapping for `[HttpEndpoint]` handlers (`HttpAppErrorMapper` maps `Unauthorized`→401/`Forbidden`→403, `ElarionHttpResults`). Also owns `AddElarionHttpJson()` — the base HTTP-transport JSON wiring that mirrors the canonical `IElarionJsonSerialization` config (naming knobs + source-gen `TypeInfoResolverChain`) onto `Microsoft.AspNetCore.Http.Json.JsonOptions`. The `[HttpEndpoint]` transport (de)serializes **both directions** through those options — request-body binding and the success value (`ElarionHttpResults.ToResult` uses `TypedResults.Ok`, so there is no custom response serializer) — so under reflection-off both need the alignment; it also keeps REST output identical to JSON-RPC/MCP for the same DTO. A deliberate global, idempotent, registration-order-overridable alignment, called by any HTTP host or by `AddElarionOpenApi`.
...
- `Elarion.AspNetCore.OpenApi` — opt-in OpenAPI document generation for `[HttpEndpoint]` handlers, the REST analog of the JSON-RPC schema/client (see [ADR-0026](docs/decisions/0026-openapi-http-transport.md)). A thin sibling of `Elarion.AspNetCore` over `Microsoft.AspNetCore.OpenApi`: `AddElarionOpenApi([documentName], configure)` calls `AddOpenApi(...)` and registers the Elarion transformers, and the host serves it with `app.MapOpenApi()`. Elarion owns only the ~20% Microsoft can't — (1) **canonical-JSON wiring**: it mirrors the canonical `IElarionJsonSerialization` knobs + source-gen `TypeInfoResolverChain` onto `Microsoft.AspNetCore.Http.Json.JsonOptions` (which OpenAPI schema-gen and minimal-API body binding read), so schemas resolve reflection-free and match the other transports' wire shape; (2) an **operation-id document transformer** that normalizes the handler-FQN operationId to a clean name (de-colliding); (3) an **idempotency operation transformer** that, for endpoints the generator marked with the inert `ElarionIdempotentEndpointMetadata`, adds an optional `Idempotency-Key` header parameter + the `x-elarion-idempotent: true` vendor extension (the OpenAPI analog of the JSON-RPC `idempotent` flag). Module grouping comes from the generator's `.WithTags("{Module}")`. The only package referencing `Microsoft.AspNetCore.OpenApi` (which pins `Microsoft.OpenApi` ≥ 2.7.5 past a transitive advisory); depends on `Elarion.AspNetCore` + `Elarion.Abstractions` and the ASP.NET shared framework. Client generation is off-the-shelf (`openapi-typescript` + `openapi-fetch`, or Kiota), and build-time export reuses Microsoft's `Microsoft.Extensions.ApiDescription.Server` — Elarion ships no OpenAPI MSBuild package.
...
- `Elarion.Generators` — Roslyn source generators for handlers, services, modules, the module-scoped RPC/HTTP/MCP transport maps (`AppModuleDiscoveryGenerator`), validation resolvers, resilience policies, scheduled jobs, event consumers, and the permission catalog (`PermissionCatalogGenerator` — the per-module runtime `IPermissionCatalog` contributions plus the compile-time `ElarionPermissions` static, from `[RequirePermission]`/`[RequireRole]`; cross-assembly entries ride the `ElarionManifest`), plus the per-module `ConfigureDefaultServices` aggregation (`ModuleDefaultServicesGenerator`) that auto-wires and feature-gates all of a module's registrations. `HandlerRegistrationGenerator` also **auto-attaches the authorization decorator** as the outermost functional gate (just inside tracing) by inspecting the handler for `[Require*]` attributes — opt-in by default, or to every non-`[AllowAnonymous]` handler under an in-scope `[ElarionAuthorizationDefaults]` — reporting `ELAUTH001` when the handler's response cannot represent failure. It likewise **auto-attaches the feature-gate decorator** (`FeatureGateDecorator`) just inside the authorization gate for any handler carrying `[FeatureGate]`, reporting `ELFEAT001` (response cannot represent failure) and `ELFEAT002` (gate with no feature name), and **auto-attaches the framework `ValidationDecorator`** just inside the feature gate (tracing → authorization → feature gate → validation → `[DefaultPipeline]` → handler) for any handler whose request type graph carries DataAnnotations validation attributes — zero cost when unannotated — reporting `ELVAL001` (response cannot represent failure, mirroring `ELAUTH001`) and `ELVAL002` (warning: request carries validation attributes but `Elarion.Validation` is not referenced — schema-documented but unenforced). The sibling `ValidationResolverGenerator` (conditional on the `Elarion.Validation` reference) walks each module's handler request graphs and emits a per-module `IValidatableInfoResolver` whose typed infos return **constant-constructed attribute arrays** (no runtime attribute reflection), registered through the module's gated `ConfigureDefaultServices` `AddValidators` hook. For **variant service injection** (ADR-0019) it wraps any handler whose constructor depends on a `[FeatureVariant]` contract in the `AsyncResolvedHandler` proxy (warming the variant before construction), and the sibling `VariantServiceRegistrationGenerator` emits the keyed/binding/transparent-factory/provider registrations (`ELVAR001`–`ELVAR007`; `[FeatureVariant]` is a modifier on `[Service]`, which `ModuleServiceRegistrationGenerator` then skips so the variant path owns the registration). Also hosts the cross-module communication tooling: the `ModuleApiGenerator` (a typed in-process API per `[GenerateModuleApi]` facade) and the `ModuleBoundaryAnalyzer` (the repo's first `DiagnosticAnalyzer`, `ELMOD002`). See [Cross-module communication](#cross-module-communication). **Not published as a standalone package** (`IsPackable=false`); its analyzer DLL is bundled into the `Elarion` runtime package.
...
- `@swimmesberger/elarion-jsonrpc-client-generator` — TypeScript CLI/library that converts exported Elarion JSON-RPC schemas into method contracts, constraint-aware Zod params/result schemas (`rpcParamsSchemas` + `rpcResultSchemas`), and a portable fetch client that **pre-validates request params by default** (`validateParams: false` to opt out; a local failure throws `RpcParamsValidationError`, batch items included). Zod v3 and v4 compatible. Lives in `src/elarion-jsonrpc-client-generator`.
...
- `Elarion.AspNetCore.OpenApi` is the only package that references `Microsoft.AspNetCore.OpenApi`; the base HTTP transport and the generator stay OpenAPI-free. The generator's `.WithTags("{Module}")` and the inert `ElarionIdempotentEndpointMetadata` marker it emits for `[Idempotent]` `[HttpEndpoint]` handlers are ignored by a host that does not reference the OpenAPI package. Do not add an Elarion OpenAPI MSBuild package or a bespoke HTTP client generator — build-time export reuses `Microsoft.Extensions.ApiDescription.Server` and client generation is off-the-shelf (ADR-0026).
...
- Request validation follows the same seam/impl split: the `IRequestValidator` seam, `RequestValidationErrors`, and the `ValidationDecorator` live in `Elarion.Abstractions` (no `Microsoft.Extensions.Validation`/`Microsoft.Extensions.Options` dependency); the M.E.Validation-backed default lives in the opt-in `Elarion.Validation`, the only package (besides generated resolver code) that touches that experimental surface. The declarative constraint vocabulary is standard `System.ComponentModel.DataAnnotations` on request DTOs — there is **no Elarion validation attribute** and **no FluentValidation dependency anywhere in the framework** (FluentValidation remains possible as an app-owned decorator only). Business rules (cross-field, conditional, async/DB) belong in handlers, never in a pre-handler validator layer (see [ADR-0027](docs/decisions/0027-declarative-request-validation.md)).
...
- **Discover through the syntax provider, never off `CompilationProvider`.** Use
  ...
  `CreateSyntaxProvider` when the trigger is a base type or structural (handlers). **Never**
...
3. Each emitted lambda binds the request (`[AsParameters]` for GET/DELETE and opt-in custom-bound requests; JSON body for default POST/PUT/PATCH), **resolves the handler typed-directly** (`[FromServices] IHandler<TRequest, Result<TResponse>>` — HTTP does **not** go through the named bus, since the route already pins the type), and translates `Result<T>` via `ElarionHttpResults` — `200`/`204` on success, RFC 7807 ProblemDetails on failure with the status from `HttpAppErrorMapper`. Each lambda also carries OpenAPI metadata: `.WithName`/`.WithDescription`, `.Produces<T>(200)`/`.Produces(204)` + `.ProducesElarionErrors()`, the owning module as an OpenAPI tag (`.WithTags("{Module}")`), and — for an `[Idempotent]` handler — the inert `ElarionIdempotentEndpointMetadata` marker. That metadata is what the optional `Elarion.AspNetCore.OpenApi` package turns into a served OpenAPI document (see [ADR-0026](docs/decisions/0026-openapi-http-transport.md) and the [OpenAPI transport model](#openapi-model)); it is inert without that package.
...
OpenAPI is the **REST transport's contract**, the analog of the JSON-RPC schema + TypeScript client, shipped as
the opt-in `Elarion.AspNetCore.OpenApi` over `Microsoft.AspNetCore.OpenApi` (see
[ADR-0026](docs/decisions/0026-openapi-http-transport.md)). It is deliberately thin — no bespoke generator, no
Elarion OpenAPI MSBuild package.

1. A host adds `AddElarionOpenApi([documentName], configure)` and serves it with `app.MapOpenApi()`. The package
   registers `AddOpenApi(...)` plus the Elarion transformers, and calls **`AddElarionHttpJson()`** (the base
   HTTP-transport wiring in `Elarion.AspNetCore`) which **mirrors the canonical `IElarionJsonSerialization`
   configuration onto `Microsoft.AspNetCore.Http.Json.JsonOptions`** — the naming knobs and the source-generated
   `TypeInfoResolverChain`. OpenAPI schema generation, minimal-API request-body binding, **and** the success value
   (`ElarionHttpResults.ToResult` uses `TypedResults.Ok` — no custom response serializer) all read those HTTP JSON
   options (not `IElarionJsonSerialization` directly), so bodies, responses, and the emitted schemas resolve through
   the same source-gen contexts with reflection off (a missing `[JsonSerializable]` throws rather than reflecting),
   matching the wire shape the other transports emit (both go through STJ's `JsonSchemaExporter`).
   `AddElarionHttpJson()` is a
   base-transport concern, not OpenAPI-specific — any `[HttpEndpoint]` host under reflection-off calls it (or
   `AddElarionOpenApi`); it is idempotent, a deliberate **global** alignment of the app's minimal-API JSON, and
   overridable by a host `ConfigureHttpJsonOptions(…)` registered after it (registration order wins).
2. The generator already annotates each `[HttpEndpoint]` (`.Produces`, `.ProducesElarionErrors()`, `.WithName`,
   `.WithDescription`, `.WithTags("{Module}")`, and the inert `ElarionIdempotentEndpointMetadata` for
   `[Idempotent]`). The package adds two transformers: an **operation-id document transformer** (normalizes the
   handler-FQN operationId to a clean, de-collided name) and an **idempotency operation transformer** (for a
   marked endpoint, adds an optional `Idempotency-Key` header parameter + the `x-elarion-idempotent: true` vendor
   extension — the OpenAPI analog of the JSON-RPC schema's `idempotent` flag). HTTP idempotency enforcement stays
   the pipeline `IdempotencyDecorator` fed by `UseElarionIdempotencyKey`; the transformer only documents it.
3. Client generation is **off-the-shelf** (`openapi-typescript` + `openapi-fetch`, or Kiota / `@hey-api/openapi-ts`);
   a small documented `openapi-fetch` middleware reads `x-elarion-idempotent` and auto-attaches the
   `Idempotency-Key` header, giving the JSON-RPC client's exactly-once-on-retry behavior. Build-time export reuses
   Microsoft's `Microsoft.Extensions.ApiDescription.Server` (`OpenApiGenerateDocuments=true`), which host-launches
   like the JSON-RPC schema tool; do not add an Elarion OpenAPI MSBuild package. Keep the OpenAPI seam out of core:
   the attribute/marker and the generator stay OpenAPI-free, and only this package references `Microsoft.AspNetCore.OpenApi`.
...
6. **Configuration-selected variants** (ADR-0028) are the process-global sibling of variant injection —
   selection by *what is configured* rather than *who is asking*. `[ConfigurationVariant("Email:Backend",
   Value = "office365")]` (also a modifier on `[Service]`; a contract uses exactly one axis, `ELVAR008`)
   selects the implementation by a plain `IConfiguration` value, matched case-insensitively with fallback to
   the default (no `Value`). The read is **synchronous**, so no async proxy or per-scope warm-up is involved:
   handlers keep the plain registration, any consumer (any assembly) injects the contract directly, and each
   new DI scope observes the current value while in-flight scopes stay pinned. Any configuration provider
   drives it — `appsettings.json` (`reloadOnChange`), environment variables, or the settings bridge
   (`AddElarionSettingsConfiguration` + `Elarion.Settings.PostgreSql`) for admin-writable, cluster-propagated
   runtime switching with **no flag provider at all**. Manual wiring:
   `AddElarionConfigurationVariantService<T>(key, defaultKey)` + lower-case keyed registrations; blank key is
   `ELVAR009`. **Choose the axis by who decides**: an answer that can differ between two concurrent requests
   is `[FeatureVariant]`; one answer per process is `[ConfigurationVariant]` — and when in doubt, start
   configuration-selected (the smaller machine; moving to the feature axis later touches only the
   implementation attributes).
7. **The variant registry** (ADR-0029) makes every switch enumerable with zero hand-kept lists. A **named
   default** (`Value` + `IsDefault = true`, both axes) is the fallback *and* explicitly selectable, so the
   whole vocabulary is declared (two defaults per contract → `ELVAR001`). `VariantCatalogGenerator`
   (`[UseElarion]`/`[GenerateVariantCatalog]`) emits the `ElarionVariants` partial static into the assembly
   root namespace — per-switch accessor classes (`Key` + value `const string`s, usable in
   `[AllowedValues(...)]`) plus `VariantDescriptor` data (`All`/`ByKey`/`ByModule`/`Platform`) — aggregated
   cross-assembly via the Elarion manifest (`Elarion.Manifest.Variant.v1`, shared `VariantDiscovery`);
   accessor-name collisions are `ELVAR010`. Variants under no module are first-class **platform** variants
   (`Module = null` — the infrastructure-adapter placement; deliberately no diagnostic), host-wired via the
   generated per-contract `Add{Contract}VariantService()`. The generator registers **nothing in DI**: the host
   seeds `AddElarionVariantCatalog(ElarionVariants.All)` (→ `IVariantCatalog`, the data bridge that lets
   application modules build their own settings APIs over platform switches without referencing them) and
   optionally `AddElarionVariantValidation()` (in `Elarion` core: startup + config-reload check that each
   configured value is offered, plus an `IServiceProviderIsService` wiring check for platform contracts;
   `Strict` fails startup). An internal contract in a referenced assembly contributes key/values with
   `Contract = null` (`ContractName` always carries the FQN). **Switch-ownership happy path** (recommend it;
...
"simple vs. complex". Tier 1 (declarative shape constraints) is one declaration feeding four surfaces;
tier 2 (business rules) lives in the handler. There is no standalone validator class layer and no
FluentValidation integration. See
[ADR-0027](docs/decisions/0027-declarative-request-validation.md) and
[the validation concept doc](docs/concepts/validation.mdx).

1. **Tier 1** is standard `System.ComponentModel.DataAnnotations` attributes on the request DTO —
   `[Range]`, `[MinLength]`/`[MaxLength]`/`[Length]`/`[StringLength]`, `[RegularExpression]`,
   `[EmailAddress]`, `[Url]`, `[Base64String]`, `[AllowedValues]` — enforced at runtime **and** exported to
   the JSON-RPC schema, MCP tool schemas, the OpenAPI document, and the generated Zod client (which
   pre-validates params). Requiredness comes from NRT + the `required` modifier (no `[Required]`). Reusable custom
   constraints subclass a mapped attribute (e.g. `[Slug] : RegularExpressionAttribute`) and get
   enforcement plus every schema surface for free.
2. **Tier 2** — cross-field comparisons, conditional rules, and every async/database-backed check —
   lives in the handler (or a domain `[Service]`), returning `AppError.Validation`/`Conflict` through
   the normal `Result<T>` channel, **inside the transaction** (a pre-handler async check is TOCTOU; the
   DB constraint is the real fence).
3. `HandlerRegistrationGenerator` auto-attaches the framework `ValidationDecorator` just inside the
   feature gate (tracing → authorization → feature gate → **validation** → `[DefaultPipeline]` →
   handler) for any handler whose request type graph carries validation attributes — zero cost when
   unannotated, pre-transaction always. `ELVAL001` (error) when the response cannot represent failure
   (mirrors `ELAUTH001`); `ELVAL002` (warning) when attributes exist but `Elarion.Validation` is not
   referenced (schema-documented but unenforced — a visible choice, never silent).
4. The decorator calls the `IRequestValidator` seam (`Elarion.Abstractions.Validation`), which returns
   `RequestValidationErrors?` with `FieldErrors` keyed by **wire-named field path** (canonical JSON
   naming policy, e.g. `"address.street"`, indexers preserved: `"deliveries[1].street"`). The HTTP
   mapper surfaces `FieldErrors` as the RFC 7807 `errors` extension; JSON-RPC carries it in
   `error.data`; a client-side Zod failure and a server 400 address the same field the same way.
5. The default implementation is `Elarion.Validation` (`AddElarionValidation`, adapts
   `Microsoft.Extensions.Validation`'s runtime base — never its generator); `ValidationResolverGenerator`
   emits a per-module `IValidatableInfoResolver` with constant-constructed attribute arrays (no runtime
   attribute reflection), registered through the module's gated `ConfigureDefaultServices` `AddValidators`
   hook, so a disabled module contributes no validation metadata.
6. Schema export: `JsonRpcSchemaExporter` injects the constraint keywords (`[Range]` →
   `minimum`/`maximum` + exclusive variants; length attributes → `minLength`/`maxLength` or
   `minItems`/`maxItems` on arrays; `[RegularExpression]` → `pattern`; `[Url]` → `format: "uri"`;
   `[Base64String]` → `format: "byte"`; `[EmailAddress]` → `format: "email"`; `[AllowedValues]` →
   `enum`); MCP tool input schemas share the builder. OpenAPI gets the same set natively from Microsoft
   (reflection-off), plus email-format and allowed-values parity transformers in
   `Elarion.AspNetCore.OpenApi`. The TS client maps the keywords to Zod (`enum` → `z.enum`/literal
   unions) and pre-validates params by default (`validateParams: false` opts out; local failures throw
...
module disappears across services, validation metadata, `MapEndpoints`, `[HttpEndpoint]`, JSON-RPC, and MCP. Handlers
...
consumers are **module-feature-gated** (a disabled module registers neither). Like handlers and
services, jobs and consumers are module-scoped only — there is no flat assembly-wide registration
...
validation resolvers, scheduled jobs, and event consumers** are registered automatically and gated, without the
...
- Runtime validation should use generated Zod schemas by default, with opt-outs or transform hooks for consumers that need them. That covers **both directions**: `rpcResultSchemas` validates results, and `rpcParamsSchemas` pre-validates request params before the wire (`validateParams: false` opts out; a local failure throws `RpcParamsValidationError`, batch items included).
- Keep the Zod emitter constraint-aware (schema keywords → `.min`/`.max`/`.regex`/`.gte`/`.lte`/`.gt`/`.lt`/`.int`/`.uuid`/`.email`/`.url`, array `minItems`/`maxItems` → `.min`/`.max`) and compatible with both Zod v3 and v4.
